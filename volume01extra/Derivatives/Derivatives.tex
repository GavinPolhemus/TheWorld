...
First, we need to know the derivatives of some simple functions. For a constant function, like $x = 1$, the slope is zero at every time, so derivative is zero.
\startuseMPgraphic{graph::xconst} % I'd like to add minor ticks, 0.667mm long.
	path xpos;
	xpos := (0,1) -- (4.4,1);
draw begingraph(4.4cm,2cm);
	setrange(origin, 4.4, 2);
	for y=0, 1, 2 :
...
endgraph;
\stopuseMPgraphic
\startplacefigure[location=margin, reference=fig:xconst, title={The function $x=1$ is a constant. The velocity $v=\ddt x$ is zero at all times.}]
\small\reuseMPgraphic{graph::xconst}
\stopplacefigure
\startformula
	\ddt 1 = 0
\stopformula
The function $x=1$ is shown in \in{figure}[fig:xconst]. The velocity $v$ is the slope, given by the derivative. (We are ignoring units until we have some building blocks. Don't panic. Units will be back soon.)

For a function that is a sloped straight line, like $x = t$, the derivative is the line's slope. The slope is the same at all times, so the derivative is the same at all times, as in \in{figure}[fig:xlin].
...
\startformula
	\ddt t = 1
\stopformula
For a parabola, like $x = t^2$, the slope is different at different locations, so the derivative is a new function. \in{Figure}[fig:xquad] shows the function $x=t^2$ and some slopes, which can be found using the derivative.
...
Find $\ddt t^3$.
\startsolution
Use the product rule with $n=3$.
\startformula
	\ddt t^3 = 3 t^{3-1} = 2t^2
\stopformula
\stopsolution
\stopexample
\startexample[ex:DerivativeConstantV]
Find $\ddt \sqrt{t}$.
\startsolution
Use the product rule with $n=\onehalf$.
\startformula
...
The coefficient $a$ will typically have units. (Yay, units!) For example, if an object's momentum is given by the formula $p = (14\units{kg\.m/s^2})\,t$, we can find the force, which is the derivative of momentum.
\startformula
	F = \ddt p
		= \ddt (14\units{kg\.m/s^2})\,t
		= (14\units{kg\.m/s^2}) \ddt t
		= 14\units{kg\.m/s^2} 
\stopformula
In the third expression, the derivative is acting on $(14\units{kg\.m/s^2})\,t$. We pull the constant $(14\units{kg\.m/s^2})$ out of the derivative to get the fourth expression, where the derivative is acting on $t$.  We then find $\ddt t$ using the power rule with $n=1$.
...
\NC	v_y \NC = \ddt y													\NR
\NC		\NC = \ddt \left[(12\units{m/s})\,t - (4.9\units{m/s^2})\,t^2\right]	\NR
...
\startformula\startmathalignment
\NC v \NC = \ddt x								\NR
\NC		\NC = \ddt (x_0 + v_0t + \half a_0 t^2)		\NR
