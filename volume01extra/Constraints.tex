...
\stopformula
There are $m$ coordinates $\eta^\alpha$ and $m$ canonical momenta $\varpi_\alpha$ ($\alpha \in \{1\dots m\}$). The constraints are maintained by the reaction force $F$, which has components $F_\alpha$.

Common constraints, like rolling, involve velocities as well as position. Rolling constraints are linear in velocities, so consider $n$ constraints of the form
\startformula
...
\startformula
	f^\mu = 0,
\stopformula
where each $f_\mu$ depends only on position. These constraints can be put in the form above by taking their time derivatives.
\startformula
