...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\startchapter[title=The New Astronomy, reference=ch:Rotation]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Galileo first explained projectile motion as the combination of uniform horizontal motion (constant \m{v_x}) with vertical free-fall motion. To Galileo, circular motion was as natural as linear motion, so he saw no obstacle to combining circular motion with linear motion. For example, he argues in \booktitle{Two World Systems} that an object dropped down a very deep well will continue to move about the Earth's center with uniform circular motion (constant \m{\omega}) as it free-falls to Earth's center.

In fact, circular motion requires a force – as Descartes and Newton taught us – to bend the path away from naturally straight motion. For example, the Moon is kept in a circular orbit around Earth by the constant pull of Earth's gravity. Earth's gravity does not simultaneously produce free-fall motion, bringing the Moon crashing down – thankfully.
...
An object's angular momentum is represented by \m{L}, and it is connected to rotational kinetic energy in the same way that momentum is connected to center-of-mass kinetic energy. Recall that we had two formulas for kinetic energy, one using velocity and the other using momentum.
\startformula
	K = \half mv^2 = \frac{p^2}{2m}
\stopformula
Likewise, rotational kinetic energy can be written in terms of angular velocity \m{\omega} or angular momentum \m{L}. Write these formulas in the same form as above.
\startformula
...
\startformula
	L = I\omega
\stopformula
This looks very similar to the momentum formula \m{p=mv}. In both cases the momentum is the product of the objects \quotation{inertia} and its velocity. This analogy between linear and angular quantities is shown in \in{table}[T:LinearAngular].
...
\HL
\NC Velocity		\NC \ \m{v}		\NC \m{\omega}		\NR
\NC Inertia		\NC \ \m{m}		\NC \m{I}			\NR
\NC Momentum	\NC \m{p=mv}	\NC \m{L=I\omega}	\NR
\LL[2]%\bottomrule
    \stoptabulate}


 The angular version of force is called torque and is represented by \m{\tau} (tau). Torque is a twisting force that will cause something to gain angular momentum. This leads to the third great conservation law of mechanics, conservation of angular momentum. 
\startformula
	L\si + \tau\Delta t = L\sf
\stopformula

We would like to explain the planets' motions around the Sun using angular momentum and energy. Orbits of the inner four planets – Mercury, Venus, Earth, and Mars – are shown in \in{figure}[fig:KeplerTerestrial]. While we could work in cartesian coordinates, the solar system is much easier to describe with polar coordinates, and this has been the tradition since before cartesian coordinates were invented. Each planet's gravitational potential energy is determined by its distance from the sun, the \m{r} coordinate. Since we will be using conservation of angular momentum, we will need to know the torque, which turns out to be zero! Hamilton's equation works with any coordinates, and for angular coordinates it gives the torque.
\startformula
	\tau = -\frac{\Delta U}{\Delta\theta}
\stopformula
Motion in the \m{\theta} direction (keeping \m{r} constant) does not change a planets' potential energy, so there is no force pushing the planets in the \m{\theta} direction. The only force is the gravitational force in the negative \m{r} direction, directly toward the Sun. The planets do exert forces on each other but these are extremely small compared to the force of the Sun's gravity, so we will ignore them.
...
{Kepler's model of Mars's orbit using a perfect circle, off center, and a constant angular velocity about an equant. The lines show the equal angle about the equant over two different \m{20\units{day}} durations. The same angle produces a larger displacement on the side farther from the equant – and closer to the Sun. This model failed.}	 % caption text
...
\startformula
	I = mr^2,
\stopformula
where \m{m} is Earth's mass and \m{r} is the radial coordinate measured as distance from the Sun, as in \in{figure}[fig:KeplerTerestrial].

Even though the Earth is spherical, we do not use the formula for a sphere's moment of inertia, the way we would for rolling problems. Earth is engaged in two circular motions: spinning about its axis and revolving around the Sun. Hamilton's formulation allows us to treat those as separate motions. To study Earth's rotation about its axis, we would use the moment of inertia of a sphere. Here, we will study only Earth's revolutions about the Sun, for which we should treat Earth as a small mass at the end of a very long rod from the center. For this motion the momentum of inertia is \m{I=mr^2}.

As we described at earlier in the chapter, Hamilton's equations tell us that there is no torque pushing Earth around in its orbit, therefore the angular momentum \m{L=I\omega} is constant. Kepler told us that Earth's angular velocity changes as it gets slightly closer to the Sun on one side of its orbit and then slightly farther away on the other. How does Earth's angular velocity \m{\omega} change if the angular momentum \m{L} remains constant? The angular velocity changes because the moment of inertia changes! Putting Earth's moment of inertia \m{I=mr^r} into the angular momentum formula gives 
\startformula
	L = mr^2\omega.
\stopformula
This angular momentum stays constant. As Earth gets slightly closer to to the Sun on one side of its orbit, the distance \m{r} decreases slightly and the angular velocity \m{\omega} must increase slightly. Earth speeds up. As Earth gets farther from the Sun again on the other side of its orbit, \m{r} increases slightly and \m{\omega} must decrease slightly. Earth slows down. This is exactly the amount of speeding and slowing required to both maintain a constant angular momentum \m{L} and to maintain equal areas swept out in equal times.
...
\stopformula 
I've added a \m{\theta} subscript to remind us that \m{K_\theta} is the kinetic energy due to motion in the \m{\theta}-direction. The angular momentum \m{L} in the formula remains constant throughout the Earth's orbit, but the moment of inertia changes, which then changes the kinetic energy. As Earth gets slightly closer to the Sun on one side of its orbit, \m{m} and \m{I} both decrease slightly, making the kinetic energy \m{K_\theta} increase.

Although the change in \m{r} is relatively small, the change in Earth's kinetic energy is huge! Where dose all of all of that energy come from? It comes from a decrease in the gravitational potential energy \m{U}.
\startformula
	U = -G\frac{mM_\Sun}{r}
\stopformula
As the distance \m{r} decreases slightly, \m{K_\theta} increases and \m{U} decreases as shown in the energy diagram in \in{figure}[fig:EnergyGraphKU]. The diagram only shows the \m{r} coordinate, not the angle \m{\theta}. The Earth is always also moving in the \m{\theta} direction as it orbits the Sun.
...
\startaxis
 [	x={2cm},y={.5cm},
   ...
   xlabel={Distance from Sun \m{r} (\m{\sci{11}\units{m}})},
   xmin=0, xmax=5,
   minor x tick num=4,
   ylabel={Energy (\m{m\sci{8}\units{J/kg}})},
   ymin=-10, ymax=6,
   minor y tick num=3,
 ]
% \addplot[ % Us
...
\addplot[ % Ug
  ...
  {-13.3/x}
  %[yshift=-10pt]
    node[below right,pos=0.9] {\m{U=-G\frac{mM_\Sun}{r}}}
  ;
...
\addplot[ % K angular Earth
  ...
  {9.926/x^2}
  %[yshift=8pt]
    node[above, pos=0.95] {\m{K_\theta=\frac{L^2}{2mr^2}}}
  ;
\addplot[ % K angular Earth
   thin,
   domain=0.5:5,
  ...
  {(-13.27/x)+(9.926/x^2)}
  %[yshift=8pt]
    node[above right,pos=0.75] {\m{K_\theta + U}}
  ;
\addplot[ % E Earth
   very thick,
   domain=1.471:1.521,
...
{Earth's rotational kinetic energy \m{K_\theta} and potential energy \m{U} plotted as functions of the distance from the Sun \m{r}. The sum \m{K_\theta+U} cradles Earth at its minimum. \m{U} prevents Earth from leaving the solar system, while \m{K_\theta} keeps it from spiraling into the Sun. Earth is always also moving the the \m{\theta} direction (not shown) as it orbits the Sun.}	 % caption text
{\noindent\typesetbuffer[TikZ:EnergyGraphKU]} % figure contents

Why does Earth not just keep spiraling inward towards the Sun, losing potential energy and gaining kinetic energy as it meets a fiery doom? This is an interesting story that requires a closer look at the energy graph in \in{figure}[fig:EnergyGraphKU]. The plots of \m{K_\theta} and \m{U} are nearly opposites, but there is a very important difference in their shapes. The potential energy \m{U} is inversely proportional to the distance \m{r} while the rotational kinetic energy \m{K_\theta} is inversely proportional to \m{r^2}.
\startformula
	U\sim \frac{1}{r} \qquad K \sim \frac{1}{r^2}
\stopformula
When \m{r} is very small, \m{r^2} is much smaller than \m{r}, making \m{K_\theta} much larger than \m{U}. When \m{r} is very large, \m{r^2} is much larger than \m{r}, making \m{K_\theta} much smaller than \m{U}. \m{K_\theta} is the dominant form of energy when \m{r} is small, and \m{U} is the dominant form of energy when \m{r} is large. This change can be seen in the sum \m{U+K_\theta} in the energy diagram (\in{fig.}[fig:EnergyGraphKU]). At small \m{r} the sum rises like \m{K_\theta}, while at large \m{r} it rises like \m{U}.

Somewhere in the middle is a minimum which cradles Earth. Earth rocks gently in this minimum, losing potential energy \m{U} and gaining rotational kinetic energy \m{K_\theta} as it moves slightly towards the Sun on one side of its orbit, then losing \m{K_\theta} and gaining \m{U} is it moves away again on the other side.
...
Mars's off-center orbit also leads to a more interesting energy graph, shown in \in{figure}[fig:EnergyGraphKUH]. Mars's energy graph shows the total energy \m{H}, including the kinetic energy \m{K_r} due to Mars's motion in the \m{r}-direction. The total energy \m{H} is slightly higher than the minimum of \m{K_\theta+U}, allowing Mars to oscillate over a significant range of distances from approximately \m{r\sub{min}=2\units{Gm}} to \m{r\sub{max}=2.5\units{Gm}}. These turning points in the energy graph are same maximum and minimum distances of Mars's orbit shown in \in{figure}[fig:KeplerTerestrial]. If you look closely at Earth's energy graph (\in{fig.}[fig:EnergyGraphKU]) you will see that it is also a small line segment showing the Earth's range of distances from the Sun.
...
\addplot[ % K angular Mars
  ...
  {(-13.27/x)+(14.99/x^2)}
  %[yshift=8pt]
    node[below left,pos=0.45] {\m{K_\theta + U}}
  ;
\addplot[ % E Mars
   very thick,
   domain=2.067:2.492,
  ...
  {-2.912}
 % [yshift=10pt]
    node[below, pos=0.5] {\m{H = U + K_\theta + K_r}}
...
{Mars's energy graph. The sum \m{K_\theta+U} cradles Mars at its minimum. Mars oscillates between approximately \m{r\sub{min}=2\units{Gm}} and \m{r\sub{max}=2.5\units{Gm}}, in agreement with the orbit shown in \in{figure}[fig:KeplerTerestrial].}	 % caption text
...
Kepler starts over again, using his table of twenty-eight distances. While considering these distances, he was playing around with another calculation and stumbled upon the number \m{1.00429}, which reminded him of one of the distances, which was off of the circle by \m{0.00432}. \quotation{It was as if I were awakened from a sleep to see a new light\dots.} % Kepler p407
...
triumphal chariot with a laurel wreath – a crown for Kepler to honor his victory over the war god, Mars.} {\externalfigure[chapter07/Kepler1609Urania][width=\rightmarginwidth]}
...
\startpolaraxis
 [	xticklabels=\empty,
 	ytick={0,5,...,45},
 	yticklabels={{},{},\m{1\units{Tm}},{},\m{2\units{Tm}},{},\m{3\units{Tm}},{},\m{4\units{Tm}},{}},
 	minor y tick num={4},
	% yminorgrids=true,
	hide x axis,
	ymax = 45,
...
{The elliptical orbits of all eight planets and Halley's Comet. The planet's orbits are nearly circular, but off-center. Halley's Comet's orbit is highly elliptical, brining it near Earth approximately every \m{76} years. It's next appearance will be in 2061. (\m{1\units{Tm} = 10^{12}\units{m}})}	 % caption text
...
Many planet's orbits do rotate slowly due to the slight pull of other planets. By 1859 it became clear that only Mercury's obit rotates in a way that cannot be caused by other planets. This anomaly was not explained until 1915, when Albert Einstein was developing his own geometric model of gravity. For eight years, Einstein had been struggling to build his theory theory of curved space-time. In the final month, he discovered that his emerging theory predicts a slightly different force than Newton's theory near the Sun. Einstein computed the effect on Mercury's orbit and found it exactly matched the well know anomaly in Mercury's orbit. He recalls, \quotation{for a few days, I was beside myself with joyous excitement.} % Pais SitL p.253
...
\stopformula
where \m{T_1} and \m{a_1} are the period and radius of one planet's orbit, while \m{T_2} and \m{a_2} are those of the other. For elliptical orbits the radius is half of the ellipse's long axis. Formally, \m{a} is called the semi-major axis.
...
\HL
\NC	 Coordinate	\NC \m{s}			\NC \m{\theta}			\NC \m{s=r\theta}	\NR
\NC Velocity		\NC \ \m{ds=v\,dt}	\NC \m{\omega}			\NC \m{v=r\omega}	\NR
\NC Inertia		\NC \ \m{m}			\NC \m{I}				\NC \m{I=mr^2}		\NR
\NC Momentum	\NC \m{p=mv}		\NC \m{L=I\omega}		\NC \m{L=rp}		\NR
\NC Force		\NC \m{dp=F\,dt}		\NC \m{dL = \tau\,dt}		\NC \m{\tau=rF}		\NR
\NC Kinetic Energy	\NC \m{K=\onehalf mv^2=\frac{p^2}{2m}}
									\NC \m{K=\onehalf I\omega^2=\frac{L^2}{2I}}
															\NC \m{K = K}				\NR
\NC Hamilton's Eq.	\NC \m{F=-\frac{\Delta U}{\Delta s}}
									\NC \m{\tau=-\frac{\Delta U}{\Delta\theta}}	\NC	\NR
\LL[2]%\bottomrule
    \stoptabulate%}
