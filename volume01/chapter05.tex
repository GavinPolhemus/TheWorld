...
Once the pendulum is released it starts moving to the right. Its gravitational potential \m{U} decreases and its kinetic energy \m{K} increases. The increasing kinetic energy is shown by the curve labeled \quotation{\m{K}} on the graph in figure~\in[fig:GalileoPendulumGraphHK].
Since no work is being done on the system by outside forces, the total energy \m{H=0.24\units{J}} stays constant, as shown by the horizontal line labeled \quotation{\m{H}} on the graph in figure~\in[fig:GalileoPendulumGraphHK]. As the potential energy decreases, the kinetic energy increases by exactly the same amount, so that the total energy \m{H=U+K} stays constant.
...
The stretch must always be measured from the relaxed length of the spring. If the spring is compressed rather than stretched, as in the ball bounce example above, then the \quotation{stretch} is negative. However, since the stretch is squared it does not matter whether the stretch is positive or negative; the energy stored is positive either way.
