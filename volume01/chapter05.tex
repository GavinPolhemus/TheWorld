...
Huygens, who discovered the conservation of \m{mv} and \m{mv^2} in pendulum collisions, also studied the colliding pendulums' initial and final heights, and again he discovered something quite remarkable.
Pendulums of different masses may be released from various heights. Then, after the colliding with each other, those masses will ascend to other various heights. Huygens discovered that for elastic collisions the total of the masses times their heights (total \m{mh}, where \m{h} is the mass's height) is the same before and after the collision.

Huygens then related the total \m{mh} at release to the total \m{mv^2} at the collision, showing that the total \m{mh} lost in the masses' descent is proportional to the \m{mv^2} right before the collision. Likewise the total \m{mv^2} right after the collision is proportional to the total \m{mh} gained in the masses' ascent.
...
The complicated systems studied by Huygens and D.~Bernoulli all store energy as \keyterm{gravitational potential energy}. Luckily, we can understand gravitational potential energy by applying D.~Bernoulli's insights to Galileo's simple pendulum experiments. A version of Galileo's pendulum with a \m{150\units{g}} mass is shown in figure~\in[fig:GalileoPendulum1].
...
%	\draw (-4.3,-0.5) rectangle (4.3,4.5); % Border
	...
	] (-4.3,0) --node[sloped,above=5mm]{\m{h} (cm)} (-4.3,4);
	% U axis
	\draw[
		postaction={decorate},
		decoration={
	...
	] (4.3,0) --node[sloped,below=6mm]{\m{U} (J)} (4.3,4.082);
...
%	] (0,0) arc[start angle=270, end angle=180, radius=5cm];
	% Pendulum
	\draw[thick] (0,4) --node[sloped,above]{\m{40\units{cm}}} (-3.2,1.6); % String
...
{A smaller version of Galileo's pendulum. Both the heigh \m{h} and the gravitational potential energy \m{U} are shown for this \m{150\units{g}} pendulum.}	 % caption text
...
The symbol for any type of potential energy is \m{U}. The formula for gravitational potential energy is
\startformula
	U = mgh.
\stopformula
Along with Hugyens' \m{mh}, this formula includes \m{g}, which is the same \m{9.8\units{m/s^2}} found in the formula for gravitational force, \m{F = mg}. The \m{g} is included because Earth's gravitational attraction makes this energy storage passible. On the moon, where \m{g} is smaller, lifting an object requires less work, but it also stores less gravitational potential energy.
...
We will measure the hight \m{h} from the equilibrium point at B, as shown on the left side of figure~\in[fig:GalileoPendulum1]. At C the balls height is \m{16\units{cm}}, so its gravitational potential energy is
\startformula
	U = mgh
		...
		= 0.24\units{J}
\stopformula
The final answer has energy units, as it should. The pendulum at C has \m{0.26\units{J}} of gravitational potential energy.
...
\startexample[ex:GalileoPendulum1] How much work did Galileo do on the \m{150\units{g}} pendulum when he moved it from its equilibrium position at B to the point C, \m{16\units{cm}} higher?
\startsolution
	Following Daniel Bernoulli, we start with the law of conservation of energy. The total energy includes both kinetic and potential energy, that is \m{H\si=K\si+U\si} and \m{H\sf=K\sf+U\sf}.
...
\stopmathalignment\stopformula
	Galileo did \m{0.24\units{J}} of work on the pendulum when he moved it from B to C.
\stopsolution
\stopexample

Whenever you are asked to calculate work you should consider using the work formula \m{W = F_x\Delta x + F_y\Delta y + F_z\Delta z}, but that would have been a poor choice here. You do not know the force Galileo applied and it almost certainly was not a constant force, making this into a calculus problem. Conservation of energy only requires the direct calculation of initial and final energies, which is much easier.

The height \m{h} can be measured from the floor, a table top, sea level, or any other convenient reference, as long as that reference stays in place for the entire calculation. A convenient choice is often the lowest point of the motion, like the point B along the path of Galileo's pendulum. The pendulum's gravitational potential energy, calculated from the ball's height, is shown for all heights along the right side of figure~\in[fig:GalileoPendulum1]. Using a different reference point, like the floor, shifts all of the heights on the left and all of the potential energies on the right. However, this would not force Galileo to do more work. No matter where the reference point is, he moves the pendulum from B to C, increasing the height by \m{16\units{cm}}. This increases the potential energy by \m{0.24\units{J}}, so he still does \m{0.24\units{J}} of work on the pendulum.
...
The descending pendulum's gravitational potential energy will decrease during the descent, and the kinetic energy will increase by the same amount. All of this is accounted for using the law of conservation of energy, this time without any outside force doing work.
	...
	\NC	v	\NC = \sqrt{2gh}
				...
				= 0.89\units{m/s}
	\stopmathalignment\stopformula
The ball passes through B at \m{0.89\units{m/s}}.
\stopsolution
\stopexample
...
While both methods are acceptable, you should include the gravitational potential energy \m{U} in the system.
Daniel Bernoulli taught that including \m{U} in the system is both easier and gives us a better insight.
...
During the descent and assent the pendulum's energy is split between gravitational potential energy and kinetic energy. The gradual decrease in gravitational potential energy during the descent and the increase during the assent are graphed in figure~\in[fig:GalileoPendulumGraphU]. The energy is shown as a function of the ball's position \m{s} along the curved path, as shown in figure\in[fig:GalileoPendulumPath1]. The gravitational potential energy is determined by the height of each position, and can be graphed for all positions along the path. The equilibrium point is the point of lowest potential energy at \m{s=0\units{cm}}.
...
\starttikzpicture% tikz code
	\startaxis[
		...
		\addplot[thick, domain=-50:50] {0.588*(1-cos(deg(x/40)))}node[above right,pos=.1]{\m{U}};
...
Galileo began his experiment by moving the pendulum from the equilibrium position at \m{s=0\units{cm}} to the position \m{s=-37\units{cm}}, as show in figure~\in[fig:GalileoPendulumPath2]. The gravitational potential energy at this release position \m{U=0.24\units{J}}, as shown on the graph in figure~\in[fig:GalileoPendulumGraphHK]. At the moment of release the pendulum's kinetic energy is \m{K=0\units{J}}, so its total energy is \m{H = U + K = 0.24\units{J} + 0\units{J} = 0.24\units{J}}, which is also shown on the energy graph in figure~\in[fig:GalileoPendulumGraphHK].
...
{Galileo's pendulum at the release position \m{s=\units{-37cm}}.}	 % caption text
...
\starttikzpicture% tikz code
	\startaxis[
		...
		\addplot[thick, domain=-50:50] {0.588*(1-cos(deg(x/40))}node[above right,pos=.06]{\m{U}};
		%\addplot[thick, domain=0:75] {0.2205*(1-cos(deg(x/15))};
		\draw[thin](-37,0) --node[pos=.6, below, sloped]{Release Position} (-37,.7);
%	\ifprintanswers
		\addplot[thick, domain=-37:37] {0.235}node[above, pos=.25]{\m{H}};
		\addplot[thick, domain=-37:37] {0.235-0.588*(1-cos(deg(x/40)))}node[below right, pos=.25]{\m{K}};
%		\addplot[thick, red, domain=0:23] {0.22-0.2205*(1-cos(deg(x/15))};
...
{The energy graph for Galileo's experiment where he releases the pendulum at \m{s=-37\units{cm}}.}	% caption text
{\noindent\typesetbuffer[TikZ:GalileoPendulumGraphHK]} % figure contents

Once the pendulum is released it starts moving to the right. Its gravitational potential \m{U} decreases and its kinetic energy \m{K} increases. The increasing kinetic energy is shown by the curve labeled “\m{K}” on the graph in figure~\in[fig:GalileoPendulumGraphHK].
Since no work is being done on the system by outside forces, the total energy \m{H=0.24\units{J}} stays constant, as shown by the horizontal line labeled “\m{H}” on the graph in figure~\in[fig:GalileoPendulumGraphHK]. As the potential energy decreases, the kinetic energy increases by exactly the same amount, so that the total energy \m{H=U+K} stays constant.

When the pendulum reaches the equilibrium point \m{s=0\units{cm}}, the potential energy is completely gone. All of the energy is kinetic, as can be seen by the point where the graph of \m{K} touches the line \m{H} on the graph.

As the pendulum continues upwards, the potential energy increases and the kinetic energy decreases. When the pendulum reaches the position \m{s=37\units{cm}}, all of the energy is again gravitational potential energy, where the \m{H} line touches the graph of \m{U} in figure~\in[fig:GalileoPendulumGraphHK]. The pendulum does not have enough energy to go any further. The kinetic energy cannot be negative, so the gravitational potential energy cannot be greater than \m{H}. The point where the \m{H=U} is called a turning point, because the pendulum will reach that point and then turn around moving in the other direction, back towards lower potential energy.
...
As the pendulum swings back and forth between the turning points and \m{s=-37\units{s}} and \m{s=37\units{cm}} the energies will continue to follow the curves in figure~\in[fig:GalileoPendulumGraphHK], going right and then left between the turning points.
...
%During the descent from C to B the ball's initial \m{0.24\units{J}} of gravitational potential energy is converted to kinetic energy. Then, during the ascent the \m{0.24\units{J}} of kinetic energy is converted back to gravitational potential energy, just enough to bring the ball back to its original height at D. In this experiment Galileo's “impetus” is clearly the ball's kinetic energy which increases during the descent from C to B and which is sufficient to carry the ball up the ascent from B to D.
%
...
\startexample[ex:BalloonLaunch] In their quest for knowledge, physics students launch a water balloon from the top of the school onto the soccer field below. The water balloon has a mass of \m{0.50\units{kg}} and is launched with with a speed of \m{15\units{m/s}} and an angle \m{53\degree} above the horizontal, as shown in figure~\ref{fig:BalloonLaunch}. What is the water ballon's speed when it hits the ground?
...
The potential energy stored in a spring depends on the spring's stiffness \m{k} and the amount of stretch of the spring \m{s}. (The spring stiffness is often called the spring constant.)
\highlightbox{
...
\pic at (-13,0){cart};
%\pic at (49,0){block};
\draw[decorate,decoration={coil,segment length=5pt}] (-46,2.5) --node[above=3pt] {\m{k}} (-19,2.5);
...
Again we will consider the combined system of the ball and the spring: \m{H = K+U}. The initial position is at full stretch (\m{K=0}, \m{s=A}) and the final position is at the midpoint (\m{U=0}, \m{v=v\sub{max}}).
\startformula\startmathalignment
\NC	H\si + \cancel{W} + \cancel{Q}	\NC = H\sf					\NR
...
Combine this formula for \m{v\sub{max}} with the formula for \m{v\sub{max}} that we found above using conservation of energy.
\startformula
	\cancel{A}\sqrt{\frac{k}{m}} = \frac{2\pi \cancel{A}}{T}
\stopformula
Cleaning this up gives a useful formula an oscillator's period \m{T}.
\highlightbox{
\startformula
	T = 2\pi\sqrt{\frac{m}{k}}
\stopformula
}
The period \m{T} depends on the mass of the cart and the stiffness of the spring, but it does not depend on the amplitude of the oscillations. If the oscillations are small, the speed is also small and each cycle takes time \m{T}. If the oscillations are big the speed is also big and each cycle is still completed in the same time \m{T}.

This can be used to find the frequency as well
\startformula
	f = \frac{1\units{cyc}}{T} = \frac{1\units{cyc}}{2\pi}\sqrt{\frac{k}{m}}
\stopformula
Again, the frequency does not depend on the amplitude, only on the mass and the spring constant. A stiffer spring (larger \m{k}) will increase the frequency, while a larger mass will lower the frequency.
...
The gravitational potential energy formula earlier in this chapter, \m{U=mgh}, is only useful for relatively small heights near the Earth's surface. Over the long distances between planets, moons, and the sun, the gravitational potential energy is a bit more complicated. The gravitational potential energy of any two objects in the solar system is
\startformula
	U = -G\frac{mM}{r},
\stopformula
where \m{m} and \m{M} are the objects' masses, \m{r} is the distance between the objects' centers, and \m{G=6.67\sci{-11}\units{m^3/kg\.s^2}} is the universal gravitational constant.
...
The second thing to notice is that the distance is between the objects' centers. When calculating the gravitational potential energy of an object on the Earth's surface the distance \m{r} is Earth's radius (\m{r\sEarth=6.37\sci{6}\units{m}}) not the height above Earth's surface.
...
Finally, the universal gravitational constant \m{G} is extremely small, so the gravitational potential energy between every-day objects at every-day distances can be completely ignored. At least one of the objects must have a huge mass in order for the gravitational potential energy to be significant. Earth's mass of \m{5.97\sci{24}\units{kg}} is enough to have a significant effect.

\startexample[] How much work is required to lift a \m{3.0\units{kg}} box from Earth's surface to a very distant location?
...
\startaxis
 [footnotesize, width=2.13in, height=2in,
   xlabel={\m{r} (\m{\sci{6}\units{m}})},
   xmin=0, xmax=50,
   ylabel={\m{U} (\m{\sci{6}\unit{J}})},
   ymin=-200, ymax=0,
   %ytick={-10,-8,...,0},
 ]
 \addplot[
...
{The gravitational potential energy of a \m{3.0\units{kg}} object in Earth's vicinity. The potential is not shown for locations inside Earth (the  gray region).}	% caption text
...
\startsolution
	...
	\stopmathalignment\stopformula
	Moving the \m{3\units{kg}} box from Earth's surface to a distant location requires a tremendous amount of work.
\stopsolution
\stopexample

\startexample[] The \m{3.0\units{kg}} box was placed far from Earth, but even so it eventually is pulled back by Earth's gravitational attraction. How fast is the box going when it enters the atmosphere, approximately \m{100\units{km}} above Earth's surface?
\startsolution
...
The final distance between Earth's center and the box is Earth's radius plus
	...
	\stopmathalignment\stopformula
	The box enters the atmosphere with a speed of about \m{11\units{km/s}}, causing it to burn up before hitting the ground.
\stopsolution
\stopexample
...
%[Example problem: Make a plot of the gravitational potential around a \m{10\units{kg}} bowling ball. Make the plot from the surface of the bowling ball at \m{r=0.10\units{m}} to a distance \m{r = 1\units{m}} from the center of the of the ball.]
%\placefigure[margin][] % location
%{}	% caption text
%\hspace{-7pt}
%%{\starttikzpicture
%%\datavisualization [scientific axes, x axis={attribute=r,
%%label={[node style={inner sep=0pt,outer sep=0pt, below = 1mm}]\m{r} (m)}, include value=0, length=3.9cm},
%%y axis={attribute=Phi, label={[node style={inner sep=0pt,outer sep=0pt}] \m{\Phi} (\m{\sci{-9}\unit{m^2/s^2}})}, include value=0,include value=-8, ticks={step=2}},  visualize as smooth line]
...
%   xlabel={\m{r} (m)},
%   xmin=0, xmax=1,
%   ylabel={\m{\Phi} (\m{\sci{-9}\unit{m^2/s^2}})},
%   ymin=-8, ymax=0,
%   ytick={-8,-6,...,0},
% ]
% \addplot[
...
%The gravitational potential in the vicinity of a \m{10\units{kg}} bowling ball. The potential is extremely small, and can only be detected with the most sensitive instruments. The potential is not shown inside the bowling ball (the gray region).
%}
...
%\label{sec:freefall}
%
%Let's see what each of these can tell us about the motion of the cart, starting with conservation of energy. In this case we will consider the cart's kinetic energy, \m{K}, and gravitational potential energy, \m{U}.
%\startformula\startmathalignment[m=2,distance=2em]
%	H\sf \NC = H\si + \cancel{W} + \cancel{Q}	\NC \NC \text{no outside work or heat}\NR
%	K\sf + \cancel{U\sf} \NC = K\si + U\si		\NC \NC \text{set \m{U=0} at the ground}\NR
%	\half mv\sf^2 \NC = \half mv\si^2 + mgy\si	\NC \NC \text{formulae for \m{K} and \m{U}}\NR
%	\half v\sf^2 \NC = \half v\si^2 + gh		\NC \NC \text{divide both sides by \m{m}}\NR
%	v\sf^2 \NC = v\si^2 + 2gh					\NC \NC \text{multiply both sides by 2}\NR
%	v\sf \NC = \pm\sqrt{v\si^2 + 2gh}			\NC \NC \text{square root of both sides}
%\stopmathalignment\stopformula
%When taking the square root of in the last line, we must include a plus-or-minus. The mathematics cannot tell us which sign to pick, but often the correct sign is obvious from the problem. For example, if an object is falling to the ground, then the final velocity in negative, so replace the \m{\pm} with a negative sign.
%
%
...
%
%In fact, an object's weight, \m{F\sub{G}} is proportional to the object's mass, \m{m}, and it is always downward. On Earth the force vector is
%
...
%	\caption{When the force is constant, the velocity at \m{t_\frac{1}{2}} is the average velocity. This makes it easy to update the position.}
%
%
