...
\section{Angular momentum for circular orbits}
...
The gravitational potential energy graph is the same shape for every planet, but the relative size of the angular kinetic energy hill in the middle of the graph depends on the planet's angular momentum. Greater angular momentum increases the size of this hill, called the \keyterm{centrifugal barrier}.
A larger centrifugal barrier pushes the planet's orbit farther out from the Sun, as shown in \in{figure}[fig:EnergyGraphKUH]. Less angular momentum produces a smaller centrifugal barrier, allowing the orbit to be closer to the Sun, as for Mercury. The size of the centrifugal barrier is different for each planet, giving their orbits different sizes in the same gravitational potential energy well.
...
{Energy graphs for the four terrestrial planets. The dotted line shows the energy and radius of perfectly circular orbits. Earth and Venus have nearly circular orbits. }	% caption text
...
Take a moment to locate Venus and Earth, and compare the energy graphs for these two planets. Earth's is the same $U+K_\theta$ curve shown in \in{figure}[fig:EnergyGraphKU].
The $U+K_\theta$ curve for Venus is a similar shape, but shifted down toward lower energy and left toward smaller $r$.
This shift is due to Venus's lower angular momentum.
Lower angular momentum gives Venus a lower angular kinetic energy $K_\theta = L^2/2mr^2$, which is why Venus's $U+K_\theta$ curve in \in{figure}[fig:EnergyGraphKUH] is lower than Earth's.

Like Earth, Venus sits right near the minimum of its $U+K_\theta$ graph, always staying nearly the same distance from the Sun in a nearly circular, centered orbit. The dotted line in \in{figure}[fig:EnergyGraphKUH] shows the radius and energy of every possible circular, centered orbit. (See \in{figure}[fig:CircularH], \at{p.}[fig:CircularH].) This dotted line passes right through the minimum of every $U+K_\theta$ curve. Earth and Venus sit on this line, since they are both in nearly circular, centered orbits.

Mercury has much less angular momentum, so its $U+K_\theta$ curve is even lower and farther left than Venus's, putting Mercury much closer to the Sun. Unlike Venus and Earth, Mercury's orbit is quite eccentric. Mercury does not sit right at the minimum of its $U+K_\theta$ curve. Mercury's total energy $H=U+K_\theta+K_R$, shown by the thick, short, horizontal line labeled \quotation{Mercury,} is somewhat higher than the minimum of the $U+K_\theta$ curve. This looks very much like the energy graph for a simple harmonic oscillator, and Mercury does indeed oscillate in the $r$ direction as it orbits the Sun.
Mercury's oscillations have a period of 88 days.

The endpoints of Mercury's total energy line are Mercury's turning points. The left end of the line represents Mercury's closest approach to the Sun, and the right end represents Mercury's greatest distance from the Sun. This range of distances is shown by the range of positions for Mercury at the top of \in{figure}[fig:EnergyGraphKUH].
Between the turning points, Mercury is moving in the $r$ direction as well as in $\theta$ direction. Mercury's radial motion requires some kinetic energy $K_r = \onehalf mv_r^2$. No outside force does any work on Mercury, so Mercury's total energy $H=U+K_\theta+K_R$ is constant as Mercury oscillates between the turning points.

Mars has the most angular momentum of the planets in \in{figure}[fig:EnergyGraphKUH], making it farthest from the Sun.
Like Mercury, Mars has an eccentric orbit and does not sit at the minimum of the $U+K_\theta$ curve. The thick, horizontal line labeled \quotation{Mars} shows Mars's total energy $H$ as it oscillates between the two turning points on the the $U+K_\theta$ curve.
Mars oscillates over a significant range of distances from approximately $r\sub{min}=2\units{Gm}$ to $r\sub{max}=2.5\units{Gm}$, as shown by the range of Mars positions above the energy graph. These turning points in the energy graph are same maximum and minimum distances of Mars's orbit shown in \in{figure}[fig:KeplerTerestrial]. 
The minimum of Mars's $U+K_\theta$ curve is much wider and shallower than the minimum of Mercury's curve, so Mars oscillates much more slowly that Mercury. Mars's oscillations have a period of 687 days.

If you look closely at Earth's energy graph in \in{figure}[fig:EnergyGraphKU] or \in[fig:EnergyGraphKUH], you will see that Earth's total energy $H$ is also a small line segment showing the Earth's small range of distances from the Sun.
Venus's range of distances is even smaller, but is still barely visible in \in{figure}[fig:EnergyGraphKUH].
...
Mars's full energy graph is shown in \in{figure}[fig:UKMars3D]. The orbit's eccentricity is clearly visible. The extra radial kinetic energy $K_r$ responsible for this eccentric motion has raised the total energy $H$ above the $U + K_\theta$ minimum, as if some water (shown in white) had been added to the moat around the central centrifugal barrier. As Mars circles the Sun, its orbit (shown in black) crosses this moat between the outer shore at $r\sub{max}$ on the right side of the centrifugal barrier, and the inner shore at $r\sub{min}$ on the left side of the centrifugal barrier. 
...
\noindent
Mercury's energy graph in \in{figure}[fig:EnergyGraphKUHMercury] shows $r_0$, $r\sub{min}$, $r\sub{max}$ and $a$ for Mercury's eccentric orbit.
