...
\Initial{J}{ohannes Kepler,} at the age of 23, became interested in astronomy for a simple reason: it was his job. He had been studying for priesthood, but he also loved mathematics. Before is graduation in the spring 1594, he received an unexpected invitation to teach mathematics and astronomy at the school in Graz, Austria.
...
Kepler's \quotation{drinking cup}\autocite{Described in a letter to Friedrich, Duke of Wuerttemberg, 27 Feb.~1596. v.~\convertnumber{KR}{13} p.~50.}{KeplerGW} model in \in{figure}[fig:Kepler1596system] looks similar to our energy graph in \in{figure}[fig:U3D]. This three dimensional energy graph will allow us to understand the planets'  two dimensional orbits. The vertical axis in our energy graph is energy, as usual. \in{Figure}[fig:U3D] shows the planets' gravitational potential energy, which is very negative near the Sun and rises toward zero far away.
\startformula
	U = -G\frac{mM}{r}
\stopformula
We will use $m$ for the mass of a planet and $M$ for the much larger mass of the Sun.

In \booktitle{Mysterium Cosmographicum}, Kepler argued that the Sun is the driver of the Solar System's motions. Earlier models were driven by many moving spheres, but in Kepler's model, the spheres are not physical. Kepler's heliocentric view of the Sun's physical importance matches our energy graph. The Sun produces the huge potential energy well that holds the much lighter planets in their orbits. (The Sun is about one-thousand times more massive that Jupiter, the largest planet.)

The six planets known to Kepler are shown at the correct distances along the $r$-axis on the energy graph. Notice how these distances approximately match the distances in Kepler's model. The  four inner planets – Mercury, Venus, Earth, and Mars – are quite close together. Jupiter and Saturn are much further out.
...
\stopformula
Mercury's orbit takes only 88 days. Venus's orbits in 225 days. Earth's orbit is 365 days. Mars's orbit is 687 days. The outer planets' orbits are far longer – nearly twelve years for Jupiter's orbit and almost thirty years for Saturn's.
...
Using the orbit radii from \in{figure}[fig:VisiblePlanets], you can calculate these periods, and you will get good results. However, in the calculations above we took an illegal short-cut by assuming circular, centered orbits and uniform circular motion. The orbits are actually off center, or \keyterm{eccentric}, as can be seen for most of the planets in \in{figure}[fig:VisiblePlanets]. The planets' speeds are also not constant. As a planet gets closer to the Sun on one side of its orbit, the planet's gravitational potential energy decreases and its kinetic energy increases, increasing its speed. As the planet gets farther from the Sun on the other side of its orbit, some of its kinetic energy gets converted back to gravitational potential energy, and the planet slows down. This speeding up and slowing down repeats with every orbit. The planet's motion is not uniform circular, it is compound motion combining angular motion around the Sun with oscillating radial motion, alternately approaching and receding from the Sun. 
...
Kepler's mysterious area law provides an interesting challenge for any theory of motion. Newton, to show the power of his laws of motion, takes on this challenge in Proposition 1 of the \booktitle{Pricipia}, where he proves the area law using geometric methods. Euler and Lagrange offered new perspectives on this law in their formulations of mechanics. We will explain the area law using Hamilton's method, which is ideally suited to the orbital motions of the solar system. In the process, we will find a new, useful conservation law and a practical method for solving equations of motions.

\section{Conservation of angular momentum}
We will describe the planets' motions with the astronomers' coordinates $r$ and $\theta$ in \in{figure}[fig:dA], with the Sun at the center. These are called \keyterm{polar coordinates}.
Hamilton's method is ideally suited to these curved coordinates, and the equations of motion will simplify nicely.
...
The most challenging part of Hamilton's method is writing the system's total energy in terms of positions and momenta. For planetary orbits, there are three contributions to the total energy: the gravitational potential energy $U$, the kinetic energy $K_\theta$ due to the planet's motion around the Sun in the $\theta$-direction, and the kinetic energy $K_r$ due to any motion toward or away from the Sun in the $r$-direction.
\startformula
	H = K_r + K_\theta + U
\stopformula
...
\startformula
	K_r = \frac{p_r^2}{2m}
\stopformula
Similarly, the kinetic energy's angular part can be written in terms of the \keyterm{angular momentum} $p_\theta$. However, when we work with an angular coordinate, the mass in the denominator is replaced by the moment of inertial $I$. 
\startformula
	K_\theta = \frac{p_\theta^2}{2I}
\stopformula
Angular momentum $p_\theta$ is a new type of momentum associated with angular motion like rotation or orbits. Euler and others developed special methods for working with angular quantities, but Hamilton's method treats angular coordinates and momenta just like any others. 
...
\startformula
	H = \frac{p_r^2}{2m} + \frac{p_\theta^2}{2mr^2} - G\frac{mM}{r}
\stopformula
The Hamiltonian contains the coordinate $r$, the momenta $p_r$ and $p_\theta$, and several constants ($m$, $M$, $G$, and $2$). These are the allowed ingredients in a good Hamiltonian. The coordinate $\theta$ is also allowed, but it does not appear.

Now we can follow Hamilton's method to find the equations of motion. Since Kepler first discovered his area law for the angular motion, we will look at the angular equations of motion first as well. The angular momentum update formula is especially simple because there are no outside forces and the coordinate $\theta$ does not appear in the Hamiltonian, allowing us to immediately use the constant rule for derivatives!
\startformula
	dp_\theta = \left(-\ppth H + \cancel{F_\theta} \right)dt = 0
\stopformula
The angular momentum does not change – it is a \keyterm{constant of the motion}. The planet may speed up and slow down as its distance from the Sun changes, but the angular momentum $p_\theta$ remains constant.

The position update formula for $\theta$ is almost as easy. Only the second term in the Hamiltonian contains the angular momentum $p_\theta$ so the other terms can be eliminated from the update formula using the constant rule.
\startformula
	d\theta = \pp{p_\theta} H\,dt
		= \frac{\partial}{\partial p_\theta}
			\left( \frac{p_\theta^2}{2mr^2} \right)dt
		%= \frac{1}{2mr^2} \pp{p_\theta} p_\theta^2\,dt
		%= \frac{1}{2mr^2} (2 p_\theta^{2-1})\,dt
		= \frac{p_\theta}{mr^2}\,dt
\stopformula
Even as the angular momentum remains constant, the size of the angular steps $d\theta$ change as the planet moves closer and farther from the Sun. The angular steps are larger when the planet is closer to the Sun and $r$ smaller; the angular steps are smaller when the planet is farther from the Sun and $r$ is larger – exactly the speeds Kepler sought to model – first with the equant, and then with his constant area law.
...
\startformula
	dA = \half (r)(r\,d\theta)
		= \half r^2\,d\theta.
\stopformula
We calculated the update $d\theta$ using Hamilton's method, and we can use it here.
\startformula
	dA = \half \cancel{r^2}\frac{p_\theta}{m\cancel{r^2}}\,dt.
		= \frac{p_\theta}{2m}\,dt.
\stopformula
Since the angular momentum $p_\theta$ is a constant of the motion, this is exactly Kepler's area law stating that equal areas $dA$ are swept out in equal times $dt$. Conservation of angular momentum, which we discovered using Hamilton's method, was first discovered by Kepler as his area law.

Angular momentum works with any angular coordinate. Many common systems are best described using angular coordinates – a swinging pendulum, a spinning top, pulleys, gears, and more. Angular momentum is not always a constant of the motion. For a pendulum, the potential energy $U$ depends on the angle $\theta$, so its angular momentum changes, always in the direction toward equilibrium. For a gear, the neighboring gears exert an outside force $F_\theta$ (called \keyterm{torque}) that can alter the gear's angular momentum. In all these cases, use $K_\theta = p_\theta^2/2I$ and follow Hamilton's general method to find the equations of motion. 
%
%The $\theta$ update formula produced by Hamilton's method is
%\startformula
%	d\theta = \frac{p_\theta}{I}\,dt.
%\stopformula
%The angular velocity in this update formula is $\omega = p_\theta/I$, which leads to a general formula for angular momentum.
%\startformula
%	p_\theta = I\omega
%\stopformula
...
\addplot[ % K angular Earth
  ...
  {9.926/x^2}
  %[yshift=8pt]
    node[above right=0mm, pos=0.7] {$K_\theta=\frac{p_\theta^2}{2mr^2}$}
  ;
\addplot[ % K angular Earth
   thin,
   domain=0.5:3,
...
\startformula
	U + K_\theta = -G\frac{mM}{r} + \frac{p_\theta^2}{2mr^2}
\stopformula
...
\section{From a circle to an ellipse}
...
We would like to relate the orbit's size and eccentricity to the planet's angular momentum $p_\theta$ and total energy $H$. Before tackling these relations, let us return briefly to circular orbits.
...
{Mercury's energy graph. The sum $U + K_\theta$ cradles Mercury at its minimum. Mercury oscillates between approximately $r\sub{min}=2\units{Gm}$ and $r\sub{max}=2.5\units{Gm}$, in agreement with the orbit shown in \in{figure}[fig:KeplerTerestrial].}	 % caption text
