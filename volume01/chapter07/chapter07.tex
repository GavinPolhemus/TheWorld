...
For a circular orbit and a constant distance $r$, the constant angular momentum would produce a constant angular velocity. But the planets' orbits are eccentric, $r$ is not constant, and the angular velocity changes as the planets get nearer to the Sun and then farther away. A planet's angular momentum is 
\startformula
  L = I\omega = mr^2\omega.
\stopformula
Solving for $\omega$ allows us to find the correct position update formula for $\theta$.
\startformula
...
\startformula\pagereference[eq:AreaUpdate]
	dA = \half \cancel{r^2}\frac{L}{m\cancel{r^2}}\,dt.
		= \frac{L}{2m}\,dt.
\stopformula
Since the angular momentum $L$ is a constant of the motion, this is exactly Kepler's area law stating that equal areas $dA$ are swept out in equal times $dt$. Conservation of angular momentum was first discovered by Kepler as his area law.
