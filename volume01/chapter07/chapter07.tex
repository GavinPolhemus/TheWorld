...
Kepler's area law can be written as an area update formula.
\startformula
	dA = (\text{const.})\,dt,
\stopformula
...
To use Hamilton's method, we need a coordinate system to describe the motion. We will use the astronomers' coordinates $r$ and $\theta$ in \in{figure}[fig:dA]: $r$ is the distance from the Sun and $\theta$ is the angle measured around the Sun. These are  called \keyterm{polar coordinates}. The equations of motion will simplify nicely in polar coordinates.

In polar coordinates, the small area swept out by a planet is a long, thin triangle, with length $r$ and width $r\,d\theta$. The triangle's area is
\startformula
	dA = \half (r)(r\,d\theta)
		= \half r^2\,d\theta.
\stopformula
We will use Hamilton's method to find the position update formula for $d\theta$.
%In order to match Kepler's area law, the small steps $d\theta$ will need to be bigger when $r$ is smaller, and smaller when $r$ is larger, so that the changes $dA$ are always equal in equal times.
...
\startformula
	H = K_r + K_\theta + U
\stopformula
The gravitational potential energy is especially simple in polar coordinates, since it only depends or the distance $r$, not on the angle $\theta$.
\startformula
	U = -G\frac{mM}{r}
\stopformula

The kinetic energy's radial part $K_r$ can be written in terms of the radial momentum $p_r$.
\startformula
	K_r = \frac{p_r}{2m}
\stopformula
The kinetic energy's angular part can be written in term of the \keyterm{angular momentum} $p_\theta$. Angular momentum is a new type of momentum associated with angular motion like rotation or orbits. Euler and others developed special methods  for working with angular quantities, but Hamilton's method treats angular coordinates and momenta just like any other coordinates and momenta. 
