...
To use Hamilton's method, we need a coordinate system to describe the motion. We will use the astronomers' coordinates $r$ and $\theta$ in \in{figure}[fig:dA], with the Sun at the center. These are  called \keyterm{polar coordinates}. The equations of motion will simplify nicely in polar coordinates.
...
The moment of inertial $I$ should be familiar from the formula for rotational kinetic energy in \in{Chapter}[ch:VisViva] (\at{p.}[eq.Krot]). There, we described an object's moment of inertia as the total $mr^2$ for all parts of the object. Planets in orbit can be treated as a single part at the distance $r$ from the Sun.
\startformula
	I = mr^2
\stopformula
Even though the Earth is spherical, we do not use the formula for a sphere's moment of inertia, the way we would for rolling problems. Earth is engaged in two circular motions: spinning about its axis and revolving around the Sun. Hamilton's formulation allows us to treat those as separate motions. To study Earth's rotation about its axis, we would use the moment of inertia of a sphere. Here, we will study only Earth's revolution about the Sun, for which we should treat Earth as a small mass at the end of a very long rod from the center. For this motion the momentum of inertia is $I=mr^2$.

We can now assemble the complete Hamiltonian for an orbiting planet. The Hamiltonian is somewhat intimidating, but notice all of the familiar parts.
\startformula
	H = \frac{p_r^2}{2m} + \frac{p_\theta^2}{2mr^2} - G\frac{mM}{r}
\stopformula
The Hamiltonian contains the coordinate $r$, the momenta $p_r$ and $p_\theta$, and several constants ($m$, $M$, $G$, and $2$). This are the allowed ingredients in a good Hamiltonian. The coordinate $\theta$ is also allowed, but it does not appear.
...
\startformula
	dp_\theta = \left(-\ppth H + F_\theta \right)dt = 0
\stopformula
...
The position update formula for $\theta$ is almost as easy. Only the second term in the Hamiltonian contains the angular momentum $p_\theta$ so the other terms can be eliminated from the updated formula using the constant rule.
\startformula
...
\stopformula
Even as the angular momentum remains constant, the size of the angular steps $d\theta$ change as the planet moves closer and farther from the Sun. The angular steps are larger when the planet is closer to the Sun and $r$ smaller; the angular steps are smaller when the planet is farther from the Sun and $r$ is larger – exactly the changes in speed that Kepler sought to model first with the equant and then with his constant area law.
...
\section{Most this section will be replaced by Hamilton's method}
