...
\NC  \cancel{m}\,r\si^2\omega\si \NC= \cancel{m}\,r\sf^2\omega\sf \NR
...
Each planet's angular momentum depends on the planet's mass, but the planet's masses do not affect their orbits. To understand orbits, we will use \keyterm{specific angular momentum}, $l = L/m$.
In physics and chemistry, the term \quotation{specific} means \quotation{per mass.} (You may remember specific heats from chemistry.) Each planet's specific angular momentum depends on the planet's distance $r$ and angular velocit $\omega$.
\startformula
  l = \frac{L}{m}
    = \frac{I\omega}{m}
    = \frac{\cancel{m}\,r^2\omega}{\cancel{m}}
    = r^2\omega
\stopformula
A planet's distance and angular velocity can be determined through careful observations from Earth, making specific angular momentum relatively easy to compute. Planet's masses are harder to determine from Earth, making angular momentum much more difficult to compute.

\startexample[ex:EarthSpecificAngularMomentum]
Find Earth's specific angular momentum, $l\sEarth$, using the distance and angular velocity from \in{example}[ex:EarthDistances].
\startsolution
We will use the distance and angular velocity given for early January, when Earth is closest to the Sun.
\startformula
  l\sEarth = r^2\omega
    = (\unit{1.471e11m})^2 (\unit{2.059e-7rad/s})
    = \unit{4.455e15 m^2/s}
\stopformula
Using the distance and angular velocity for early July, when Earth is farthest from the Sun, would have given the same specific angular momentum, $l\sEarth = \unit{4.455e15 m^2/s}$.
\stopsolution
\stopexample

The units of specific angular momentum are $\unit{m^2/s}$, or area per time, suggesting a close connection with Kepler's area law!

Now we can use Earth's specific angular momentum to find the correct angular velocity $\omega$ to use in the position update formula for $\theta$. Solve $l=r^2\omega$ for the angular velocity.
\startformula
  \omega = \frac{l}{r^2}
\stopformula
Earth's specific angular momentum $l$ is constant, so Earth's angular velocity $\omega$ is greatest when $r$ is smallest, and Earth's angular velocity is smallest when $r$ is greatest.

The angular velocity allows us to find the correct position update formula for $\theta$ as the planet's distance $r$ changes.
\startformula
  d\theta = \omega\,dt
    = \frac{l}{r^2}\,dt
\stopformula
...
To see how our $\theta$ update formula compares with Kepler's area law, we will find the small area $dA$ swept out in the short time $dt$.
In polar coordinates, the small area $dA$ swept out by a planet is a long, thin triangle with length $r$ and width $r\,d\theta$, as shown in \in{figure}[fig:dA]. The thin triangle's area is
\startformula
	dA = \half (r)(r\,d\theta)
		= \half r^2\,d\theta.
\stopformula
...
\startexample[ex:EarthAreaDay]
Find area swept out by Earth in one day.
\startsolution
We will find the area $dA$ for a short time $dt = \unit{1 day} = \unit{86,400 s}$. Earth's specific angular momentum $l\sEarth = \unit{4.455e15 m^2/s}$ was found in \in{example}[ex:EarthSpecificAngularMomentum].
\startformula
	dA = \half l\sEarth\,dt
	   = \half (\unit{4.455e15 m^2/s})(\unit{86,400 s})
	   = \unit{1.92e20 m^2}
\stopformula
Each day, Earth sweeps out a huge area, $\unit{192 Gm^2}$.
\stopsolution
\stopexample
Since each small area $dA$ is the same size, many repeated steps $dt$ will produce a large area $\Delta A$ proportional to the steps' total duration $\Delta t$. As a result, we can write \keyterm{Kepler's area law} as a formula for any area $\Delta A$ swept out during any duration $\Delta t$.
\startformula\pagereference[eq:KeplersAreaLaw]
  \frac{\Delta A}{\Delta t} = \half l
\stopformula
On the left side is Kepler's area $\Delta A$ swept out during the duration $\Delta t$. On the right side is the modern concept of specific angular momentum. Both sides are constant for any orbit, no matter how eccentric it might be.
...
\stopformula
This curve increases sharply close to the Sun, due to the sharp rise in the angular kinetic energy $K_\theta$. This sharp rise is called the \keyterm{centrifugal barrier} because it pushes Earth away from the center.
(Latin \quotation{fugus} means \quotation{fleeing}, like a fugitive. The centrifugal barrier causes Earth to flee from the center.)
The curve also rises towards zero far from the Sun, where the sharper drop in angular kinetic energy allows the more persistent rise of gravitational potential energy $U$ to dominate.
...
Earth's full orbit is shown on the energy graph in \in{figure}[fig:UK3D], where the $\theta$ direction has been restored. This graph reminds me again of Kepler's early drinking cup model, where Earth's orbit was confined to a spherical shell between two platonic solids (\at{p.}[fig:Kepler1596system]). In our energy graph, Earth's orbit is held between two competing energies. Rising gravitational potential energy surrounds Earth's orbit, preventing Earth from wandering out of the Solar System.
Rising angular kinetic energy creates the centrifugal barrier inside Earth's orbit, preventing Earth from spiraling into the Sun.

\section{Specific energy graphs}
Each planet's potential and kinetic energies depend on the planet's mass, but the planet's masses do not affect their orbits. To understand orbits, we will use \keyterm{specific energy}, which is energy per mass. 
Earth's \keyterm{specific energy graph} is shown in \in{figure}[fig:SpecificEnergyGraphKU]. The specific energy graph looks almost identical to the energy graph in \in{figure}[fig:EnergyGraphKU], but the specific energy graph has several advantages.
...
{The Sun's gravitational potential $\Phi$ and Earth's specific angular kinetic energy $k_\theta$ plotted as functions of the distance from the Sun $r$. The sum $\Phi +k_\theta$ cradles Earth at its minimum.
$\Phi$ prevents Earth from leaving the solar system, while $k_\theta$ keeps it from spiraling into the Sun. Earth is always also moving the $\theta$ direction (not shown) as it orbits the Sun.}	 % caption text
{\noindent\typesetbuffer[TikZ:SpecificEnergyGraphKU]} % figure contents

First, Earth's gravitational potential energy $U$ is replaced by the Sun's gravitational potential $\Phi$.
The Sun's gravitational potential extends throughout the solar system, independent of the individual planets. Every planet is held in orbit by the same gravitational potential $\Phi$ produced by the Sun.

Second, Earth's specific kinetic energy, $k = K/m$, depends only on Earth's speed, making it easy to calculate from observations of Earth's orbit.
\startformula
  k = \frac{K}{m}
    = \frac{\onehalf\cancel{m}\,v^2}{\cancel{m}}
    = \onehalf v^2
\stopformula
The units of specific energy are $\unit{m^2/s^2}$, the same as the units of gravitational potential $\Phi$.
%The units are shown on the vertical axis of Earth's specific energy graph.

Third, Earth's specific angular kinetic energy, $k_\theta = K_\theta/m$, is easily related to Earth's specific angular momentum $l$. Since $L=ml$, Earth's angular kinetic energy is 
\startformula
  K_\theta = \frac{L^2}{2I}
    = \frac{(ml)^2}{2mr^2}
    = \frac{m^{\cancel{2}}l^2}{2\cancel{m}\,r^2}
    = m\frac{l^2}{2r^2}
\stopformula
This gives a simple \keyterm{specific angular kinetic energy formula} for $k_\theta$.% = K_\theta/m$.
\startformula
  k_\theta = \frac{l^2}{2r^2}
\stopformula
Earth's specific angular kinetic energy $k_\theta$ depends only on Earth's specific angular momentum $l$ and distance $r$, not on Earth's mass.

Finally, the curve $\Phi + k_\theta$ shows the same minimum holding Earth in a stable orbit between the rising centrifugal barrier on the left and the rising gravitational potential on the right.
Earth's total specific energy, $h = \Phi + k_\theta + k_r$, is shown in this minimum.

\in{Figure}[fig:EnergyGraphKUH] shows the specific energy graphs for the four inner planets\dash Mercury, Venus, Earth and Mars. The planets are all in the Sun's gravitational potential well, shown by the bottom curve marked $\Phi$ is \in{figure}[fig:EnergyGraphKUH].
...
\startaxis
 [	x=5cm,y={1cm},
  ...
  \addplot[ % K angular Mercury
      ...
      samples=150]
    {(-13.27/x)+(3.68/x^2)}
    node[above right, pos=0.44] {$l\substar\Mercury = \unit{2.71e15 m^2/s}$};
  \addplot[ % H Mercury
      very thick,
      domain=0.46:0.698,
      samples=2]
  ...
  \addplot[ % K angular Venus
      ...
      samples=150]
    {(-13.27/x)+(7.179/x^2)}
    node[above right, pos=0.58] {$l\substar\Venus = \unit{3.79e15 m^2/s}$};
  \addplot[ % H Venus
      very thick,
      domain=1.075:1.089,
      samples=2]
  ...
  \addplot[ % K angular Earth
      ...
      samples=150]
    {(-13.27/x)+(9.926/x^2)}
    node[above right, pos=0.55] {$l\substar\Earth = \unit{4.45e15 m^2/s}$};
  \addplot[ % E Earth
      very thick,
     domain=1.471:1.521,
     samples=2]
  ...
  \addplot[ % K angular Mars
      ...
      samples=150]
    {(-13.27/x)+(14.99/x^2)}
    node[above right, pos=0.5] {$l\substar\Mars = \unit{5.48e15 m^2/s}$};
  \addplot[ % E Mars
      very thick,
      domain=2.067:2.492,
      samples=2]
...
{Specific energy graphs for the four terrestrial planets. The dotted line shows the total specific energy of perfectly circular orbits. Earth and Venus have nearly circular orbits. }	% caption text
{\vskip1.67in\hbox{\noindent\starttikzpicture
	\draw[white] (0,0)-- ++(5,0.2); % Sky to make height better. Use \vskip6.4in for bottom.
\stoptikzpicture}}

Each planet has its own specific angular momentum $l$, which determines the size of the planets' potential barrier. 
Greater specific angular momentum increases the size of the centrifugal barrier.
...
Take a moment to locate Venus and Earth, and compare the specific energy graphs for these two planets. Earth's is the same $\Phi+k_\theta$ curve shown in \in{figure}[fig:SpecificEnergyGraphKU].
...
This shift is due to Venus's lower specific angular momentum.
Lower specific angular momentum gives Venus a lower specific angular kinetic energy $k_\theta = l^2/2r^2$, which is why Venus's $\Phi+k_\theta$ curve in \in{figure}[fig:EnergyGraphKUH] is lower than Earth's.
...
If you look closely at Earth's specific energy graph in \in{figure}[fig:SpecificEnergyGraphKU] or \in[fig:EnergyGraphKUH], you will see that Earth's total specific energy $h$ is also a small line segment showing the Earth's small range of distances from the Sun.
...
{x * cos(deg(y))}, {x * sin(deg(y))}, {(-13.27/x)+(15/x^2)}
    ...
    \node at (1.8,-2.4,-4.2) {$\Phi + k_\theta$};
    \addplot3 [
        thick,
        domain=0:2*pi,
        samples=49,
...
\marginTikZ{}{UKMars3D}{Mars's specific energy graph shows the Sun's gravitational potential well and Mars's centrifugal barrier. Mars does not sit right at the minimum of $\Phi + k_\theta$ because it has extra specific energy. This extra specific energy is like water in the moat. As Mars orbits the central Sun, it also oscillates between the two banks of the moat, making the orbit eccentric.} % vskip, name, caption
...
All that remained was to triumph over the process of getting \booktitle{New Astronomy} published\dash which took another four years.
...
triumphal chariot with a laurel wreath – a crown for Kepler to honor his victory over the war god, Mars.} {\vskip4.5in\externalfigure[Kepler1609Urania][width=\rightmarginwidth]}

Kepler believed that his mathematical model of Mars's motion – the area law on an elliptic orbit – would apply to all of the planets. He was correct. Telescopes, which were not available to Tycho or Kepler, allowed far more precise observations. Kepler's model continued to provided excellent predictions, not only for the planets' nearly circular orbits, but also for the highly elliptic orbits of comets like Halley's Comet, shown in \in{figure}[fig:HaleysComet]. 
...
{The elliptic orbits of all eight planets and Halley's Comet. The planet's orbits are nearly circular, but off-center. Halley's Comet's orbit is highly elliptic, bringing it near Earth approximately every $76$ years. It's next appearance will be in 2061. ($1\units{Tm} = 10^{12}\units{m}$)}	 % caption text
{\noindent\typesetbuffer[TikZ:HaleysComet]} % figure contents

Each planet's specific angular momentum $l$ and total specific energy $h$ can be easily found from the planet's orbit.
\startformula
	l = \sqrt{GMr_0}
	\qquad
	h = -\frac{GM}{2a}
	  = -\frac{GM}{r\sub{min} + r\sub{max}}
\stopformula
The specific angular momentum $l$ is determined by Kepler's $r_0$, the vertical distance in \in{figure}[fig:KeplerDistanceEllipse].
Total specific energy $h$ is determined by the ellipse's major axis $2a=r\sub{min} + r\sub{max}$, the horizontal distance in \in{figure}[fig:KeplerDistanceEllipse].


\section{Harmonies of the World}

Having mastered the geometry of orbits, Kepler next attempted to unite the quadrivium by finding musical relationships to explain the sizes and eccentricities of the planets' orbits. In 1621 he published his findings as \booktitle{Harmonies of the World,} a book with an amazing mixture of precise astronomical observations and calculations combined with musical notation for chords and short musical phrases.

In the early seventeenth century, explaining astronomy with music was no more fanciful than introducing ellipses.
However, ellipses have survived rigorous observations, while Kepler's musical speculations have not.
The book does contain one remarkable gem, which has come to be known as Kepler's third law. In studying the periods and sizes of the planets' orbits.
\startblockquote
...
A planet's period can be calculated directly using Kepler's area law (\at{p.}[eq:KeplersAreaLaw]) for one complete orbit. For a complete orbit, the total duration $\Delta t$ is the period $T$. The total area $\Delta A$ is the ellipses's area $A$.
\startformula
  \half l = \frac{\Delta A}{\Delta t}
    = \frac{A}{T}
\stopformula
...
\startformula
  T = \frac{2A}{l}
\stopformula
The denominator is given by the specific angular momentum formula $l = \sqrt{GMr_0}$. The numerator requires some geometry. The area of an ellipse is $A = \pi ab$.
(This area formula is easiest to see in the ellips's mathematical description in \in{figure}[fig:MathEllipse].)
It is an amusing fact about ellipses that the ratio of $a$ to $b$ is the same as the ratio of $b$ to $r_0$. 
\startformula
  \frac{a}{b} = \frac{b}{r_0}
\stopformula
This relation says that $b$ is the geometric mean of $a$ and $r_0$. Solving for $b$, we find $b=\sqrt{ar_0}$, and therefore $A = \pi a\sqrt{ar_0} = \pi a^{\threehalves}\sqrt{r_0}$. These relations simplify the formula for the orbit's period.
\startformula
  T = \frac{2A}{l}
    = \frac{2\pi a^{\threehalves}\sqrt{\cancel{r_0}}}{\sqrt{GM\cancel{r_0}}}
    = \frac{2\pi}{\sqrt{GM}}\,a^{\threehalves}
\stopformula
A planet's orbital period is proportional to the orbit's semi-major axis $a$ raised to the three-halves power. This is the modern statement of \keyterm{Kepler's third law}.
...
Newton then showed that the \threehalves\ power ratio between period and radius is also a consequence of his theories of motion and gravity. 
