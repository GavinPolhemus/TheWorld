...
An energy graph, like the ones introduced in \in{Chapter}[ch:VisViva] (\at{p.}[fig:CartWork]), clearly shows the flow of energy from gravitational potential energy to kinetic energy as the pendulum descends. The pendulum's energy graph, \in{figure}[fig:GalileoPendulumGraphU], shows the ball's gravitational potential energy $U$ as a function of the pall's position $s$ along the ball's path. Positions are labeled along the path in \in{figure}[fig:GalileoPendulumPath2].
...
\noindent
Galileo began his experiment by moving the pendulum from the equilibrium position at $s=0\units{cm}$ to the position $s=-37\units{cm}$ (where $h=16\units{cm}$), as shown in \in{figure}[fig:GalileoPendulumPath2]. The gravitational potential energy at this release position $U=0.24\units{J}$, as shown on the graph in \in{figure}[fig:GalileoPendulumGraphHK]. At the moment of release the pendulum’s kinetic energy is $K=0\units{J}$, so its total energy is $H = U + K = 0.24\units{J} + 0\units{J} = 0.24\units{J}$, which is also shown on the energy graph in \in{figure}[fig:GalileoPendulumGraphHK].
...
\startbuffer[SpringCart]
	\fill [black!10] (-.2,0) rectangle (4.8,-.15);
...
For example, pushing the cart to the left, as shown in \in{figure}[fig:SpringCartLeft] stores some potential energy in the spring, as shown in the energy graph in \in{figure}[fig:CartGraphs].
...
The cart will continue to oscillate back-and-forth between the turning points. The system's energy will continue changing from the spring's potential energy to the cart's kinetic energy and then changing back with each oscillation from one turning point to the other.
\startuseMPgraphic{graph::IdealSpringEnergy} % I'd like to add minor ticks, 0.667mm long.
	path U, H, K, TPL, TPR;
	U := (-24,24) ..controls (-8,-8) and (8,-8).. (24,24);
	H := (-12,6) -- (12,6);
	K := (-24,-18) ..controls (-8,14) and (8,14).. (24,-18);
...
\startplacefigure[location=margin, reference=fig:IdealSpringEnergy, title={The potential energy of an ideal spring is a perfect parabola.}]
\small
\reuseMPgraphic{graph::IdealSpringEnergy}
\stopplacefigure

In the last section I promised that any system's motion could be predicted using the energy graph. For the cart connected to an ideal spring, the motion is simple harmonic. All we need to determine is the motion's amplitude and period. ( I have not proven to you that this system's motion is simple harmonic. You will have to trust me for now.)

The motion's amplitude is the maximum displacement from the equilibrium point. This is easy to find on the energy graph – it is the distance to the turning points, which is determined by the initial energy.
...
draw begingraph(4.5cm,2.4cm);
	...
	gdraw(K) withpen pencircle scaled 0.8pt;
	glabel.lrt("$K$",0.4);
	gdraw(TPL) withpen pencircle scaled 0.8pt;
	glabel.top("Turning",0.6) rotatedaround(point 0.6 of TPL, 90);
	glabel.bot("point",0.6) rotatedaround(point 0.6 of TPL, 90);
	gdraw(TPR) withpen pencircle scaled 0.8pt;
	glabel.top("Turning point",0.4) rotatedaround(point 0.4 of TPR, 90);
endgraph;
...
The motion's period is the time it takes the cart to go from one turning point to the other and back. This is not easy to find on the energy graph, but we can get it with a little work. We know from \in{Chapter}[ch:Motion] that the period $T$ is related to the amplitude $A$ and the maximum velocity. We also know how to find the maximum velocity using the energy graph. Perhaps if we relate these two, we can find the period.

The spring’s maximum compression is the amplitude of the oscillation. That is the moment when all of the energy is in the spring as potential energy. When the ball passes through the midpoint of the oscillation all of the energy is in the kinetic energy of the cart. Using conservation of energy we can find the maximum speed of the cart (at the midpoint) from the amplitude.

Again we will consider the combined system of the cart and the spring: $H = K+U$. The initial position is at full stretch ($K=0$, $x=A$) and the final position is at the midpoint ($U=0$, $v=v\sub{max}$).
\startformula\startmathalignment
\NC	H\si + \cancel{W} + \cancel{Q}	\NC = H\sf					\NR
\NC	\cancel{K\si} + U\si			\NC = K\sf + \cancel{U\sf}		\NR
...
\section{Small vibrations of real systems}

Moving the cart to the right, as shown, requires some work and stores potential energy in the spring. 
The ideal spring's potential energy has a very simple formula.
\startformula[eq:1DUspring]
	U\sub{spring} = \half kx^2
\stopformula
