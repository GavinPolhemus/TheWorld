...
draw begingraph(4.5cm,2.4cm);
	...
	glabel.lft(textext("Energy (J)") rotated 90,OUT)  shifted (2mm,0);
...
%	endfor
	...
	glabel.lft(textext("$t$ (s)") rotated -90,OUT) shifted (2mm,0);
...
Objects held in place by friction do not produce simple harmonic motion. A block placed on a table will not slide off, even if the table is not perfectly level, because friction between the block and the table holds the block in place. If you displace the block a few centimeters, it will not oscillate like a pendulum. It just sits motionless at the new position. Sliding the block requires some work, but that work is not stored as potential energy. Instead, it goes to random, microscopic motion in the surface of the block and the table. (The sliding sound you hear is some of that random motion.)

Until we study heat, we will always leave this random, microscopic motion out of our systems. That means that friction will always be an external force that does work on the system – usually negative work.
...
\startuseMPgraphic{graph::BlockSHOGraphs} % I'd like to add minor ticks, 0.667mm long.
	path U, H, K, Re, TPA, TPB, TPC, TPD, TPE, TPF, St;
	U := (-24,24) ..controls (-8,-8) and (8,-8).. (24,24);
...
draw begingraph(4.5cm,2.4cm);
	...
	gdraw(TPA) withcolor "middlegray";
	glabel.rt("turn 1",1) rotatedaround(point 0.97 of TPA, 90);
...
%	glabel.rt("turn 6",1) rotatedaround(point 0.97 of TPF, 90);
	gdraw(St) withcolor "middlegray";
	glabel.urt("stop",.85) rotatedaround(point 0.97 of St, 90);
	gdraw(U) withpen pencircle scaled 0.8pt withcolor "middlegray";
	glabel.llft("$U$",0.17);
	gdraw(H) withpen pencircle scaled 0.8pt;
	glabel.top("$H$",0.36);
endgraph;
...
\startplacefigure[location=margin, reference=fig:BlockSHOGraphs, title={The block at its release position (top), the energy graph for the block and spring (middle), and the block's position vs.\ time graph (bottom).}]
\typesetbuffer[starttikz,SpringBlockClean,stoptikz]\small
\reuseMPgraphic{graph::BlockSHOGraphs}
\stopplacefigure

In \in{figure}[fig:BlockSHOGraphs]....
...
draw begingraph(4.5cm,2.4cm);
	...
	gdrawarrow(H) withpen pencircle scaled 0.8pt;
	glabel.top("$H$",0.7);
	gdraw(nope) withpen pencircle scaled 0.8pt dashed withdots;
	glabel.bot("no",1.25);
	glabel.bot("no",0.75);
	glabel(mydot,1);
...
\startplacefigure[location=margin, reference=fig:SlideToStop, title={The block stops at $x=3\units{cm}$, unable to move in either direction.}]
\small
\reuseMPgraphic{graph::SlideToStop}
\stopplacefigure

\in{figure}[fig:SlideToStop]
When the block arrives at $x=3\units{cm}$, its total energy $H$ is equal to the potential energy $U$. It cannot continue to the left because its total energy would be less than its potential energy, which is impossible. It also cannot turn and go to the right because that would also lead to its total energy being less than its potential energy.

\section{Equilibrium}

\startformula
	\partial U = F_x\,\partial x
\stopformula
