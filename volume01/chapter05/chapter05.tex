...
Recognizing these limits also helps us understand why simple harmonic oscillators are so common. 

Any time something is held in place, you are likely to have a simple harmonic oscillator – at least for amplitudes that are small enough.
...
\startluacode
	function mp.morse(r)
		local U = 2.47*(1-math.exp(2.66-r))^2
		mp.inject.numeric(U)
	end
	function mp.morseinverselow(U)
		local r = 2.66-math.log(1+math.sqrt(U/2.47))
		mp.inject.numeric(r)
	end
...
%	glabel.bot("point",0.72) rotatedaround(point 0.72 of TPR, 90);
	...
	glabel.lft("$U$",21);
	glabel.rt("simple",33);
	glabel.rt("harmonic",31.6);
	glabel.rt("motion",30);
	glabel.rt("Bang!",19.7) rotatedaround(point 19.7 of U, 10);
	glabel.urt("Bye!",63) rotatedaround(point 63 of U, 10);
	gdraw(H) withpen pencircle scaled 0.8pt;
	glabel.top("$H$",0.5);
%	gdraw(K) withpen pencircle scaled 0.8pt;
%	glabel.lrt("$K$",0.4);
...
\startformula
	T = \frac{2\pi A}{v\sub{max}} = \frac{2\pi 37\units{cm}}{1.77\units{m/s}} = 1.3\unit{s}
\stopformula
