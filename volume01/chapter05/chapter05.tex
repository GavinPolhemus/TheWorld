...
An energy graph, like the ones introduced in \in{Chapter}[ch:VisViva] (\at{p.}[fig:CartWork]), clearly the flow of energy from gravitational potential energy to kinetic energy as the pendulum descends. The pendulum's energy graph, \in{figure}[fig:GalileoPendulumGraphU], shows the ball's gravitational potential energy $U$ as a function of the ball's position $s$ along its path. Positions are labeled along the path in \in{figure}[fig:GalileoPendulumPath2].
...
The spring's \keyterm{stiffness}, represented by $k$, is a measure of how difficult it is to stretch or compress the spring. Very stiff springs are hard to stretch or compress, while springs with a small stiffness stretch or compress easily. (The spring stiffness is often called the \keyterm{spring constant.})
...
The motion's amplitude is the maximum displacement from the equilibrium point. This is easy to find on the energy graph – it is the distance to the turning points, which are determined by the initial energy. The amplitude $A$ of the cart's simple harmonic motion is shown on the position vs.\ time graph in \in{figure}[fig:CartGraphs]. You can see how the cart's release point, the turning points, and the amplitude line up. In the diagram, the cart is released at $x=-12\units{cm}$, so the turning points are at $x=\pm12\units{cm}$. The position vs.\ time graph shows the cart's motion oscillating between $x=-12\units{cm}$ and $x=12\units{cm}$. Therefore, the amplitude of the oscillations is $A=12\units{cm}$.
...
\startplacefigure[location=margin, reference=fig:I2Energy, title={Two iodine atoms are connected to form an iodine molecule, \chemical{I_2}. They are actually connected by an electron bond, but they oscillate as if they are connected by a tiny spring.}]
\small
\reuseMPgraphic{I2Energy}
\stopplacefigure
...
Until we study heat, we will always leave this random, microscopic motion out of our systems. That means that friction will always be an external force that does work on the system – usually negative work, reducing the system's energy.
...
\startbuffer[SpringBlockCleanRight]
	...
	\draw[->, very thick] (4,0) --node[below, pos=0.4] {$F$} (-3,0);
    \stopaxis
\stopbuffer
...
After the turning point, when the block is sliding back towards the left, the opposing force of friction is to the right (\in{fig.}[fig:SlideLeft]). 
...
\section{Equilibrium: the principle of virtual work}
...
\startbuffer[SpringBlockCleanStop]
	...
	\draw[->, very thick] (-3,0) --node[below, pos=0.4] {$F$} (-10,0);
    \stopaxis
\stopbuffer
...
\stopformula
Equilibrium means \quotation{equal balance} between the external forces and the change in potential energy. The principle of virtual work says equilibrium is achieved when the virtual work $F\partial x$ of the external forces $F$ is exactly enough to provide the virtual potential energy change $\partial U$ that would occur due to the virtual displacement $\partial x$.
If the principle of virtual work is not satisfied, then the system is not in equilibrium.

We can see that our block is in equilibrium at $x=-3\units{cm}$ by moving all of the virtual quantities in the principle of virtual work to one side.
\startformula
...
In the case of the block, friction is the external force that holds the system in equilibrium. This kind of equilibrium is extremely common. A coffee cub set on a nearly level table does not slide off, even if the table is slightly sloped, because friction between the table and the cup holds the cup in place. The size of this frictional force, which depends on the slope of the table, can be found using the principle of virtual work.

\startexample[ex:BoxEarthGravU] A $0.85\units{kg}$ coffee cup is placed on a table. The table is $1.6\units{m}$ wide and not perfectly level, with the right edge being $0.80\units{cm}$ higher than the left edge, as shown in \in{figure}[]. Find the friction force holding the cup in place, preventing it from sliding toward the lower edge of the table.
...
\startplacefigure[location=margin, reference=fig:CupTable, title={The block at its release position (top), the energy graph for the block and spring (middle), and the block's position vs.\ time graph (bottom). The block's kinetic energy $K$ is not shown.}]
\typesetbuffer[starttikz,CupTable,stoptikz]\small
...
\startformula\startmathalignment
	...
	\NC				\NC = mg\partial h	\NR
	\NC				\NC = mg\frac{0.80\units{cm}}{1.6\units{m}}\partial x	\NR
	\NC	F 	\NC = mg\frac{0.80\units{cm}}{1.6\units{m}}		\NR
\stopmathalignment\stopformula
The friction force is positive, pushing to the right to prevent the cup from sliding to the left.
\stopsolution
\stopexample
...
Equilibrium and the principle of virtual work also do not ensure that the equilibrium is stable. Consider the three marbles on three different surfaces in \in{figure}[fig:EquilibriumStability]. The marbles are free to roll along the surface, but are not being pushed either direction by external forces. Each marbles in equilibrium, but that equilibrium is more secure or precarious depending on the shape of the surface beneath it.
...
Objects that do not roll are held in stable equilibrium by friction. A small nudge from an external force is countered by a small change in the friction force, maintaining the stable equilibrium.

% the virtual work done by the external forces equals the resulting virtual change in potential energy for all possible virtual displacements of the system. <- Save for Ch 6.
...
\startexample[ex:BoxEarthGravU] How much work is required to lift a $3.0\units{kg}$ box from Earth’s surface to a very distant location?
