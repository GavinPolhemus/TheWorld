...
Johannes Kepler’s books on the heliocentric model were also circulating at that time. The heliocentric model quickly became accepted by nearly all astronomers in Europe.

Johannes Kepler\pagereference[NewAstronomy] – who had access to new, very precise measurements – discovered that the planets actually travel along off-center, elliptical orbits (shown in \in{figure}[fig:KeplerCompound]), not circles.
These elliptical orbits are another type of compound motion, combining each planet's motion around the Sun with another oscillating motion that brings the planet closer to the Sun on one side of the orbit and takes the planet farther from the Sun on the other side.
This motion is more complicated than anything described by Galileo. The planets' motion around the Sun is not uniform. Each planet's angular velocity increases when it is closer to the Sun and decreases when it is farther away. Each planet's motion toward the Sun is not free-fall motion (which would end with planets crashing into the Sun!), nor is it simple harmonic. Kepler published his compound motion model – along with the supporting observations and calculations – in 1609. Kepler sent a copy of his \booktitle{New Astronomy} to Galileo, but Galileo kept his attention on the relative merits of the Ptolemaic and Copernican systems, both based on circular motion. While Kepler's model can be described using Galileo's language of compound motion, actually predicting the motions will require new tools. We will return to Kepler's model in \in{Chapter}[ch:Rotation], when we have those tools in hand.
