...
Luckily, Galileo knew a few things about motion. Before his adventures with the telescope, Galileo had studied motion carefully, probably more carefully than anyone before him.
...
The diagram and table give locations at a few specific times, but the graph shows the location of the ball at every moment.
...
\marginTikZ{}{ConstVelocity}{The position vs.\ time graph for the ball rolling on a level surface.} % vskip, name, caption
...
\stopformula
The position vs.\ time graph of this straight-line formula is shown in \in{figure}[fig:linearposition].
...
In fact, you used cycles when studying the circular motion of planets and moons.
Degrees are familiar and cycles are sensible, but I want to introduce you to your new best friend: radians. 
...
\marginTikZ{\vskip3.3in}{inclineacceleratingball}{A ball's speed increases at it rolls down an incline. The ball’s position is shown at $0.1\units{s}$ intervals.} % vskip, name, caption
...
We would like to analyze motion even when the force is changing. For this, we need tools developed in the late seventeenth century, when Isaac Newton and Gottfried Leibniz invented calculus to deal with changing velocities. Newton’s calculus was built on the Galileo’s geometric methods. Leibniz’s calculus, based on algebra, proved more practical.
...
\marginTikZ{}{BallRollsDown}{The rolling ball's position vs.\ time graph shows the ball's velocity starting small (small slope) and becoming large (large slope). The small $dx$ and $dt$ can be used to accurately estimate the velocity at $t=0.50\units{s}$.} % vskip, name, caption
...
A pendulum, like the one described above, will swing with the same period whether it swings a centimeter in each direction, or 20 centimeters.
...
The central point of the motion is called the \keyterm{equilibrium point}. If the pendulum is not swinging, then it is motionless at the equilibrium point.
Figures \in[fig:PendulumLeadCork] and \in[fig:PendulumLargeSmallA] show the motion for some of Galileo’s experiments, all of which have the same period.
...
The pendulum's changing velocity can be seen as the changing slope on the position vs.~time graph.
...
\startbuffer[PendulumLargeSmallA]%
	...
	\startaxis[
		...
		clip mode=individual,
  		...
  		\draw [middlegray]({4*3.14}, 30) -- ({5*3.14}, 30);
	...
	\startaxis[
		...
		clip mode=individual,
  		...
  		\draw [<->]({4.5*3.14}, 25) --  ({4.5*3.14}, 40);
  		\draw [middlegray]({4*3.14}, 40) -- ({5*3.14}, 40);
...
Astronomy studies the planetary motions over vast expanses of time and space.
Galileo demonstrated that these motions could be understood by studying the motions of simple objects in laboratory experiments.
