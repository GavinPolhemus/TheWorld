...
Speed along these circular paths can be calculated using \quotation{distance over duration.} The distance traveled in a single cycle is the orbit's circumference, found from the orbit's radius, $R$. The cycle's duration is the orbit's period. Using the new notation introduced in this chapter, the speed of an object in uniform circular motion is
\startformula[eq:vT]
	\abs{v} = \frac{2\pi R}{T}.
\stopformula
...
Acceleration complicates our efforts to predict an object’s future position. The position update formula, $x\si + v\Delta t = x\sf$, only works if the velocity remains constant throughout the duration $\Delta t$. However, we can still succeed by focusing on a short duration $dt$ during which the velocity changes very little. During this short duration the position changes by small amount $dx$, given by
\startformula
dx = v\,dt.
\stopformula
...
The vertical motion of the ball is accelerated. To find the vertical velocity at a specific time we should use the instantaneous velocity method with very small changes in $t$ and $y$. Unfortunately, the times listed in the table are far enough apart that the velocity changes significantly with each step.
...
(This is only true if the amplitude is considerably smaller than the length of the string, which will be the case in our examples.)
...
\noindent In this example we could have used the tangent drawn in figure \in[fig:vmax]. Notice that the tangent’s $\Delta x$ is
\startformula
