...
He did not publish his new science of motion at the time, but he retained his notes. His model of motion, refined through many experimental tests, explained why Earth seems motionless even though it is rotating and and moving in orbit about the sun. In 1632, Galileo published a detailed discussion of his observations and their implications in the book \booktitle{Dialogue on the Two Chief World Systems, Ptolemaic and Copernican.}
...
Galileo shared this insight first in his \booktitle{Two World Systems}. \quotation{So, tell me,} Salviati inquires,
\startblockquote
...
The marks on the meter stick allow us to identify the ball’s position, represented by the variable \m{x}.
...
is displaced \m{5\units{cm}} then it moves in the positive direction and will end at \m{25\units{cm}}. A displacement of \m{-5\units{cm}} would move the cart form \m{20\units{cm}} to \m{15\units{cm}}. %Many important physics laws have the form \quotation{initial plus change is final.}
...
{\noindent\starttikzpicture[baseline]%
	...
	\pic at (35,0) {cart};
    \draw[thick,->] (35,2.5) --node[above=2.5mm]{\m{v=-2.4\units{m/s}}} ++(-12,0);
	  	\addlegendimage{
...
The magnitude of velocity \m{v} is the absolute value \m{\abs{v}}, which is the \keyterm{speed}. Speed is always positive for a moving object, independent of the motion’s direction. The familiar formula for constant speed is \quotation{distance over time.} In our language speed is actually \quotation{distance over duration,} where the distance is the path length. (For uniform motion the path is straight, so \m{s=\abs{\Delta x}}.)
\startformula
...
{\noindent\starttikzpicture[baseline]%
	...
	\pic at (40,0) {cart};
    \draw[thick,->] (40,2.5) --node[above=2.5mm]{\m{v=-1.8\units{m/s}}} ++(-9,0);
	  	\addlegendimage{
	  		legend image code/.code={\draw[thick,|-|](-0.5cm,0cm)--(0cm,0cm);}
 		};
...
The \quotation{rise} on the graph is the displacement of the ball, \m{\Delta x}, on the vertical axis. The \quotation{run} is the duration, \m{\Delta t}, on the horizontal axis. Velocity is \quotation{displacement over time} which is the slope, \quotation{rise over run,} of the graph.
\startformula
	v = \frac{\Delta x}{\Delta t}
		= \frac{\text{rise}}{\text{run}}
		= \text{slope}
\stopformula
Keep in mind that the displacement \m{\Delta x} can be either positive or negative. If the graph is sloped downwards, then \m{\Delta x} is negative and the velocity is negative. The \quotation{rise} arrow should point down if the displacement is negative.
...
So far, we have considered motion along a straight path. Position can also be measured along a curved path. Displacement and distance, velocity and speed along the curved path are all calculated using the same formulas as along straight paths. %In practice, curved tracks do not work quite as well due to increased friction.
...
% Margin image
\placefigure[margin][fig:Phases]{Galileo’s drawings show Venus’s phases and changes in apparent size. (\booktitle{The Assayer,} 1623])} {\externalfigure[VenusPhases][width=144pt]}
...
As we saw in Chapter \in[ch:Music], the speed along a circular path can be calculated using \quotation{distance over duration,} distance is the circumference and the duration is the period. Using the new notation introduced in this chapter, the speed of an object in uniform circular motion is
%\highlightbox{
\startformula[eq:vT]
...
\section{Angles, angular velocity, and radians}
We will usually follow the mathematicians' tradition of using positive for counter-clockwise motion and negative for clockwise.

The angular displacement \m{\Delta \theta} can be measured in cycles, degrees, or radians. These angular units are related by
\startformula
	1\units{cyc} = 360\degree= 2\pi\units{rad}.
\stopformula
Degrees have been a common choice for physicists and astronomers as well as for non-specialists. Cycles are also common. In fact, you used cycles when studying the circular motion of planets and moons. The number of cycles \m{N} is the very large angular displacement \m{\Delta \theta} written in units of cycles, \m{N = \Delta \theta}. 
...
\startformula
	\theta = \frac{s}{r}
\stopformula
Of course, \m{s} and \m{r} are both lengths and should have the same units, so the units on the right cancel out. However, you need angular units for \m{\theta} on the left. Radians cheerfully step in to provide the units. You don’t have to do anything else, but if you wish to convert to other angular units, you can.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\startexample[ex:RadiansLatitude]
Latitude and longitude are measured in degrees. Latitude goes from \m{0\degree} at the equator to \m{90\unit{\degree N}} at the North Pole (and \m{90\unit{\degree S}} at the South Pole). Florence is \m{4870\units{km}} north of the equator, as shown in \in{figure}[fig:RadiansLatitude]. What is Florence’s latitude? (Earth’s radius is \m{6370\units{km}}.)
\startbuffer[TikZ:RadiansLatitude]
\environment env_physics
\environment env_TikZ
\setupbodyfont [libertinus,11pt]
\setoldstyle % Old style numerals in text
...
\draw (2.5,0) coordinate (A) -- (0,0) coordinate (B) --node[right=1.5mm]{\m{r}} (C)
pic ["\m{\theta}", draw, angle eccentricity=1.4] {angle}
pic ["\m{s}", draw, angle radius=2.5cm, angle eccentricity=1.09] {angle};
\stoptikzpicture
\stopTEXpage
\stopbuffer

\placefigure[margin][fig:RadiansLatitude] % location
{Florence's latitude is the angle \m{\theta}.}	% caption
{\noindent\typesetbuffer[TikZ:RadiansLatitude]} % figure contents
...
\startbuffer[TikZ:RadiansEratosthenes]
\environment env_physics
\environment env_TikZ
\setupbodyfont [libertinus,11pt]
\setoldstyle % Old style numerals in text
...
\draw (-2.5,0) --node[below]{Equator} (2.5,0);
\draw (A) --node[below right,pos=.4]{\m{r}} (0,0) coordinate (B) --node[pos=.3, above]{\m{\theta}} (C)
...
\placefigure[margin][fig:RadiansEratosthenes] % location
{Measuring Earth's diameter on your bike tour of Egypt.}	% caption
{\noindent\typesetbuffer[TikZ:RadiansEratosthenes]} % figure contents
\startsolution
...
This rate is called the angular velocity, represented by \m{\omega} (omega), it is the rate at which the orientation angle \m{\theta} (theta) is changing.
\startformula
...
\stopformula
The \scaps{si} units for \m{v} on the left are \m{\unit{m/s}}. The units on the right should be converted to \m{\unit{m\.rad/s}} which turn into \m{\unit{m/s}} when the radians disappear. The units match, as they must.
...
The tangent’s \m{\Delta x} and \m{\Delta t} should be found using the points where the tangent reaches the boundary of the graph. This will give the most accurate value for the instantaneous velocity. Use this method when finding velocity from a graph.
...
\noindent In this example we could used the tangent drawn in figure \in[fig:vmax]. Notice that the tangent’s \m{\Delta x} is
\startformula
	\text{tangent’s \m{\Delta x}} = 56.4\units{cm} - (-6.4\units{cm})
		= 62.8\units{cm},
\stopformula
which is equal to
\startformula
...
%
%% Margin image
%\placefigure[margin][fig:MoonSurface]{The moon’s hills and valleys are clearly visible in Galileo’s original watercolors.} {\externalfigure[MoonPhases6][width=144pt]}
%
%% Margin diagram
