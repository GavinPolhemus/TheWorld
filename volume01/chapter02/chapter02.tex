...
%We will always calculate the amount of time elapsed \m{\Delta t} as a positive number by subtracting the initial time \m{t\si} from the final time \m{t\sf}.  (The equations still work when going backward in time, but it gets confusing.)
\startexample
...
The cart rolls along the track with a constant velocity of \m{-x1.8\units{m/s}}. It passes the \m{40\units{cm}} mark on the track at \m{t=1.00\units{s}}, as shown in figure \in[fig:CartUpdateEx]. If it continues to roll at a constant velocity, what will its position be at \m{t= 1.10\units{s}}?
\startsolution
...
\startformula
	v = \frac{\Delta x}{\Delta t}
		...
		= \text{slope}
\stopformula
Keep in mind that the displacement \m{\Delta x} can be either positive or negative. If the graph is sloped downward, then \m{\Delta x} is negative and the velocity is negative. The \quotation{rise} arrow should point down if the displacement is negative.
...
Whenever you need angular units, put in radians. If you need \m{1/\unit{rad}} or \m{\unit{rad^2}}, put it in. Afterward, convert to other angular units if you need to. If you have angular units you don’t need, convert them to radians and let them disappear. No other unit works like this. There when you need them, gone when you don’t, radians are the best!
...
The rate of change of angular displacement is \keyterm{angular velocity,} represented by \m{\omega} (omega).
\startformula
