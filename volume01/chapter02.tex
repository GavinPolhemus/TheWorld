...
The magnitude of velocity \m{v} is the absolute value \m{\abs{v}}, which is the \keyterm{speed}. Speed is always positive for a moving object, independent of the motion’s direction. The familiar formula for constant speed is \quotation{distance over time.} In our language speed is actually \quotation{distance over duration,} where the distance is the path length. (For uniform motion the path is straight, so \m{s=\abs{x}}.)
\startformula
