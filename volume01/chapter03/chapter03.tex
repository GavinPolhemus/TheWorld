...
\stopblockquote
The symbol \m{\vec{p}} represents momentum. Using our mathematical language, Newton's definition of momentum is%
%\highlightbox{%
\startformula
...
\marginTikZ{}{CartMomentumEx}{The cart’s momentum in Example\ref{ex:CartMomentum}.} % vskip, name, caption
...
\startexample[ex:CartMomentum]
	...
	The momentum of the cart is \m{-480\units{g\.m/s}}, as shown in \in{figure}[fig:CartMomentumEx].
...
\marginTikZ{}{BlockSlide}{A block with initial momentum \m{p\si} slides to a stop.} % vskip, name, caption
...
\startexample[ex:cannonballFg]
	...
	\stopformula
	The gravitational force is downward, in the negative \m{y}-direction.
\startformula
	\vec F = \components{0, mg} = \components{0,-390}\unit{N}
\stopformula
\stopsolution
\stopexample
The gravitational force is the only significant force acting on the falling rock, the ball rolling off the edge, and the cannon ball. For all of these objects the net force \m{\vec F\sn} is simply the gravitational force.
...
\stopformula
Two seconds after leaving the cannon, the ball’s momentum is \m{\vec p\sf = \components{1600,420}\unit{kg\.m/s}}, as shown in \in{figure}[fig:cannonconservepf], where $\vec p\si$ and $\vec J$ are added by the tip-to-tail method.
...
\marginTikZ{}{cannonFgFa}{The force of gravity and the resisting force of air on the cannon ball in example~\in[ex:cannonpf]. The air’s resisting force is opposite the ball’s motion} % vskip, name, caption
...
\marginTikZ{}{MoonMomentum1}{The Moon orbits Earth with a constant speed. The moon’s momentum \m{\vec p}, always tangent to the path, has a constant magnitude \m{p} but a changing direction.} % vskip, name, caption
...
\marginTikZ{}{MoonMomentum2}{Placing the momentum vectors at the same place show’s the arc produced by the small momentum changes \m{d\vec p}.} % vskip, name, caption
...
\marginTikZ{}{MoonMomentum3}{The small momentum changes \m{d\vec p} are produced by the force \m{\vec F}, which is tangent to the arc and therefore perpendicular to the momentum \m{\vec p}.} % vskip, name, caption
...
\marginTikZ{}{MoonMomentum4}{As the Moon circles Earth, the force on the Moon is always towards Earth, perpendicular to the momentum, changing the momentum’s direction but not the speed.} % vskip, name, caption
