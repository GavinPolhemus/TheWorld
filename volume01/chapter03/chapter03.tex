...
Finding momentum is simmilar. The ball in figure~\ref{fig:BallOffTableVel2} has mass $m=150\units{g}$. The momentum $\vec p$ has a magnitude of $(150\units{g})(2.3\units{m/s}) = 345\units{g\.m/s}$ and points in the same direction as $\vec v$, as shown in figure~\ref{fig:BallOffTableMomentum}.
...
\stopformula
We now take our fist baby step using the position and momentum update formulas to find the small changes $dx$, $dy$, $dp_x$, and $dp_y$.
...
\startbuffer[MoonMomentum2]
	...
	\draw[thick,->] ({-120}:3cm) --node[below left]{$d\vec{p}$} ({-130}:3cm);
...
\startbuffer[MoonMomentum3]
	...
	\draw[thick,->] ({-130}:3cm) --node[above right]{$\vec{F}$} ++({140}:2cm);
\stopbuffer
...
Third, students find momentum much easier to understand than acceleration.
