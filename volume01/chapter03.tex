...
Gottfried Leibniz also admired the contributions of Huygens, Wallis, and Wren, and he began developing his own system of mechanics. Unlike Newton, Leibniz adopted Huygens’s formula \m{mv^2} as the important quantity. Leibniz called \m{mv^2} the \textit{vis viva} or \quotation{living force} and made it the basis of his own theory of motion. The \textit{vis viva} is much closer in spirit to Descartes’s original quantity of motion because \m{mv^2} is positive for any motion and zero only for systems completely at rest, just like Descartes’s \m{mv}.
...
\startexample[ex:CartMomentum]
	...
	\startsolution
		Although it is fine to work with centimeters, we prefer meters. Convert the velocity to meters per second using the fact that the \quotation{c} in \quotation{cm} stands for \m{10^{-2}}.
	\startformula
...
\startblockquote
	{\sc Law 2 }
	\textit{A change in momentum is proportional to the impulse and takes place along the straight line in which that impulse is impressed.}\autocite{p.~416. \quotation{Motion} changed to \quotation{momentum,} \quotation{motive force impressed} to \quotation{impulse.}}{Newton1999}
\stopblockquote
...
\stopformula
Objects in our everyday lives are constantly interacting with other objects, so there are often many impulses that must be added. We will represent total of all of these impulses as \m{\vec{J}\sn}, with the \quotation{net} reminding us to catch \emph{all} the impulses and add them when finding the change in momentum.
\startformula
...
Newton also does not use any specific units, just \quotation{parts} for everything. When he says that body \m{A} \quotation{has two parts of velocity} and body \m{B} has \quotation{ten parts of velocity} he is saying that \m{B} is moving five times faster than \m{A}. A part of mass times a part of velocity is a part of momentum. (\quotation{Parts} are no longer accepted units.)% The {\sc si} units which we use had not been established in Newton’s time.)
%\placefigure[margin][] % location
%{}	% caption text
...
%\stopformula
We will solve this problem using the conservation of momentum equation, broken into components equations. (Here we drop the \quotation{net.}) First the \m{x}-component:
\startformula\startmathalignment
