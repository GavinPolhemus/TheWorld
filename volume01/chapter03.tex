...
To find the speed at the collision point, we use the formula for the maximum speed of simple harmonic motion. The amplitude is the ball's initial distance from its equilibrium point, $A=15.0\units{cm}$. (Vector notation is not required for one dimensional problems. For these I will continue to use $\abs{v}$ for speed and $v$ for velocity.)
	\startformula
		\abs{v}\sub{max} = \frac{2\pi A}{T}
			...
			= 30.0\units{cm/s}
	\stopformula
The speed does not tell us the velocity's direction, but we can see that the ball is moving the in positive direction, so the velocity is $30.0\units{cm/s}$.
\stopsolution
\stopexample
...
Gottfried Leibniz also admired the contributions of Huygens, Wallis, and Wren, and he began developing his own system of mechanics. Unlike Newton, Leibniz adopted Huygens's formula $mv^2$ as the important quantity. Leibniz called $mv^2$ the \textit{vis viva} or “living force” and made it the basis of his own theory of motion. The \textit{vis viva} is much closer in spirit to Descartes's original quantity of motion because $mv^2$ is positive for any motion and zero only for systems completely at rest, just like Descartes's $mv$.
...
\stopblockquote
The symbol $\vec{p}$ represents momentum.%
%\highlightbox{%
\startformula
...
\startexample[ex:CartMomentum]
	A cart, whose mass is $800\units{g}$, is rolling on the track with a velocity of $-60\units{cm/s}$.	 Calculate the cart's momentum.
	\startsolution
		Although it is fine to work with centimeters, we prefer meters. Convert the velocity to meters per second using the fact that the “c” in “cm” stands for $10^{-2}$.
	\startformula
	...
	The momentum of the cart is $-480\units{g\.m/s}$.
...
\stopformula
Objects in our everyday lives are constantly interacting with other objects, so there are often many impulses that must be added. We will represent total of all of these impulses as $\vec{J}\sn$, with the “net” reminding us to catch \emph{all} the impulses and add them when finding the change in momentum.
\startformula
...
{A block with initial momentum $p\si$ slides to a stop.}	% caption text
{\noindent\starttikzpicture[baseline]%
	\fill [black!10] (0,0) rectangle (4.9,-.15);
	\startaxis[
		margin cart track,
    ...
    %\draw[->, thick] (10,0.25) --node[above]{$F$} (0,0);
		% Legend
   		\addlegendimage{
	  	legend image code/.code={\draw[thick,|-|](-0.5cm,0cm)--(0cm,0cm);}
 		};
...
A block with an initial momentum of $p\si=150\units{g\.m/s}$ slides on a track. The friction with the track produces a gentle backwards force of $-0.30\units{N}$. (Negative because it opposes the block's positive velocity.) How long will it take for the block to come to a stop?
\startsolution
...
Newton also does not use any specific units, just “parts” for everything. When he says that body $A$ “has two parts of velocity” and body $B$ has “ten parts of velocity” he is saying that $B$ is moving five times faster than $A$. A part of mass times a part of velocity is a part of momentum. (“Parts” are no longer accepted units.)% The {\sc si} units which we use had not been established in Newton's time.)
%\placefigure[margin][] % location
%{}	% caption text
...
\stopblockquote
The masses are $m_A=3$ and $m_B=1$. The initial velocities are $v\sub{$A$i}=2$ and $v\sub{$B$i}=10$. Therefore, the \emph{total} initial momentum is
\startformula\startmathalignment
...
\startexample[ex:cannonpv]
	A cannon fires a $40\units{kg}$ cannon ball at an upwards angle so that its initial momentum is $\vec p\si = \components{1600, 1200}\unit{kg\.m/s}$, as shown in figure~\in[fig:cannonp]. Find the cannon ball’s initial velocity and speed.
...
In the tip-to-tail method is is easy to see that the $x$-component of the initial momentum added to the $x$-component of the impulse gives the $x$-component of the final momentum, and likewise for the $y$-components. When working with components it will be useful to break Newton's second law into component equations (dropping “net” to reduce clutter).
\startformula
...
\startexample[ex:cannonballFg]
	...
	\stopformula
	The gravitational force is downwards, in the negative $y$-direction.
\startformula
	\vec F = \components{0, mg} = \components{0,-390}\unit{N}
\stopformula
\stopsolution
\stopexample
This force is the only significant force acting on the falling rock, the ball rolling off the edge, and the cannon ball. For all of these the net force $\vec F\sn$ is simply the gravitational force.
...
\draw[thick,->] (5.28,2.86)--node[above, sloped]{$\scriptstyle \vec p\sf = ?$} ++(2.4,0.6); % Momentum
\stoptikzpicture}
	A cannon fires a $40\units{kg}$ cannon ball with a speed of $50\units{m/s}$. The ball is fired at an upwards angle so that its initial momentum is $\vec p\si = \components{1600, 1200}\unit{kg\.m/s}$, as shown in figure~\in[fig:cannonpf]. While it is in the air gravity exerts a downwards force $\vec F = \components{0, -390}\unit{N}$. %The ball encounters some resistance to moving through the air, which exerts a backward force of approximately $F\sub{air} = \components{-40, -30}\unit{N}$.
...
%\stopformula
We will solve this problem using the conservation of momentum equation, broken into components equations. (Here we drop the “net.”) First the $x$-component:
\startformula\startmathalignment
...
\stopformula
Two seconds after leaving the cannon, the ball’s momentum is $\vec p\sf = \components{1600,420}\unit{kg\.m/s}$, as shown in figure~\in[fig:cannonpfanswer].
\stopsolution
...
\draw[thick,->] (0,0)--node[above]{$\scriptstyle \vec p\si$} ++(2.4,1.8); % Momentum
\draw[thick,->] (0,0)--node[right]{$\scriptstyle \vec F = \components{0, -390}\unit{N}$} ++(0,-1.96); % Fg
...
\draw[thick,->] (2.4,1.8)--node[right]{$\scriptstyle \vec F\sn \Delta t$} +(0,-1.17); % Fnet
...
\filldraw[white] (5.28,2.86) rectangle (7.53,3.14); % Trajectory
\draw[shade, ball color = black] (5.28,2.86) circle[radius=.05cm]; % Ball final
\draw[thick,->] (5.28,2.86)--node[above, sloped]{$\scriptstyle \vec p\sf = \components{1500, 360}\unit{kg\cdot m/s}$} ++(2.4,0.6); % Momentum
\stoptikzpicture}
\stopexample
...
\draw[thick,->] (2.4,-1.2)--node[right]{$\scriptstyle \vec F\sn dt$} +(0,-0.6); % Fnet
\draw[thick,->] (0,0)--node[below left]{$\scriptstyle \vec p$} ++(2.4,-1.8); % pf
\stoptikzpicture}
...
{The Moon orbits Earth with a constant speed. The moon’s momentum $\vec p$, always tangent to the path, has a constant magnitude $p$ but a changing direction.}	% caption text
...
{Placing the momentum vectors at the same place show’s the arc produced by the small momentum changes $d\vec p$.}	% caption text
{\noindent\starttikzpicture	% tikz code
...
{The small momentum changes $d\vec p$ are produced by the force $\vec F$, which is tangent to the arc and therefore perpendicular to the momentum $\vec p$.}	% caption text
{\noindent\starttikzpicture	% tikz code
...
\startformula
	\frac{dp}{p} = \frac{ds}{R}
\stopformula
The short distance traveled by moon during a time step is $ds = v\,dt$. The magnitude of the Moon’s small momentum change is $dp = F\,dt$. Plugging these into the ratios above, we find that both sides can be divided by $dt$ allowing us to solve for the magnitude of the centripetal force $\vec F$.
\startformula\startmathalignment
