...
Ancient musicians discovered that {\em simple ratios produce consonant pitches.} For example, two pipes with lengths in the ratio of three-to-two or two-to-one (see figure \in[fig:Pythagoras]) will produce two pitches that sound good together – two consonant pitches. Pipes with less simple ratios, like nine-to-eight, will produce dissonant pitches. This \keyterm{mathematical model} of music connects the purely mathematical idea of simple ratios to the physical phenomena of consonant sounds.
...
During the late sixteenth century, new ideas and observations caused a crisis for the quadrivium disciplines.  Musicians and astronomers proposed bold new models linking the quadrivium disciplines to a new science of motion. Physicists tested and refined this theory through the seventeenth, eighteenth, and early nineteenth centuries, eventually revealing a new physics that unified the quadrivium. This book is the story of that surprising and beautiful theory of motion. In this chapter, we will lay the foundation for the new physics by studying the arithmetic model of music and the geometric model of astronomy.
...
We start with Pythagoras and his follower Philolaus, who demonstrate the consonant pitches produced by simple ratios in figure \in[fig:Pythagoras].
The lengths of the pipes are shown, so ratios can be calculated.
The longest pipes produce the lowest pitches, and the shortest pipes produce the highest.
Philolaus and Pythagoras are playing pipes whose lengths are in a two-to-one ratio, which produces a nice, consonant sound.
They hold additional pipes that can produce every simple ratio in the Pythagorean musical system.
The Pythagoreans held strong numerological beliefs about the number four being complete, so they built their musical system from the five simple ratios that can be made from the numbers one through four.
...
The size of the vibrations is called the \keyterm{amplitude}, and is represented by the symbol $A$.  Amplitude is the distance that the string moves to either side of the center, as shown in figure~\in[fig:stringamp].
The total width of the string's motion is twice the amplitude, or $2A$.  For stringed instruments, the amplitude of typical vibrations is a few millimeters or less.
The vibration’s amplitude determines the sound’s volume.
...
We will eventually understand these surprising relationships though our new physics of motion.
...
\placefigure[margin][fig:PythagoreanPipesArrows]{The frequencies of Pythagoras and Philolaus’s pipes, with arrows showing the simple frequency multipliers relating consonant pitches. Arrows that are the same length show the same frequency multiplier.} {\externalfigure[PythagoreanPipesArrowsShorter]}%[width=\rightmarginwidth]}
\noindent
...
Arrows show how the frequencies are related, with the ratio given as a frequency multiplier. The frequency where an arrow starts multiplied by the ratio is the frequency where the arrow ends. For example, the first pipe’s frequency multiplied by $4/3$ is the second pipe’s frequency.
\startformula
...
The keyboard in \in{figure}[fig:TinyPiano8th5th] shows the modern letter names of the pitches we will count.
...
For example, the eight pitches G–A–B–C–D–E–F–G make an octave from G to G. A fifth encompasses five pitches, like C–D–E–F–G, the interval from C to G. A fourth encompasses four pitches, like the interval from G up to C. A third encompasses three. A second, which encompasses only two adjacent pitches, is often called a step.

\placetable[margin][T:PyIntervals] % Label
    {Consonant intervals of the Pythagorean scale.} % Caption
...
Each interval is associated with a ratio, as shown \in{table}[T:PyIntervals].
Octaves are always a ratio of 2. Going up an octave doubles the frequency. Going down an octave cuts the frequency in half. Two frequencies separated by an octave are so consonant that they are given the same name. 
Pythagoras and Philolaus' pipes in \in{figure}[fig:PythagoreanPipesArrows] contain three octave intervals: low G to middle G, middle G to high G, and C to C. These are shown by the three arrows labeled 2 in \in{figure}[fig:PythagoreanPipesArrows].
...
The fifths in Pythagoras and Philolaus' pipe collection (C to G and F to C) are related by the consonant ratio $3/2$. Since the pitches are consonant, these fifths are called \keyterm{perfect fifths}. Perfect fifths always corresponds to the ratio $3/2$. The fourths in their collection are \keyterm{perfect fourths,} corresponding to the ratio $4/3$.

\placefigure[margin][fig:PythagoreanPipesOct]{One octave of Pythagoras and Philolaus’s pipes, with arrows showing the intervals within the octave. The largest interval is the octave  from the low C to the high C. The small, dissonant interval from F to G is a whole step.} {{\vskip0.25in}\externalfigure[PythagoreanPipesOct]}%[width=\rightmarginwidth]}
...
Most instruments are built to play many specific pitches determined by the lengths of its pipes or strings, or by the placement of holes or frets.
Ideally, many of these pitches can be combined in consonance, which means many of the frequencies produced by the instruments should be related by simple ratios. Finding such a set of pitches is a challenging mathematical problem. Many cultures have adopted different solutions, each giving their music a distinctive sound.

The ancient Greeks produced several solutions. The Pythagoreans took a strictly mathematical approach, using only ratios of one, two, three, and four. Their method was extremely influential, providing the basis for music through the middle ages. Aristoxenus, in the fourth century \scaps{bc}, took a more experimental approach, determining consonant pitches by listening to whether they sound consonant. Mathematics was a useful guide, but Aristoxenus believed music is good if it sounds good. His \booktitle{Harmonic Elements} was first ridiculed and then largely ignored by music theorists, but his ideas continued to be influential, as we will see.
...
Several representations of the C major scale are shown in figure \in[fig:TinyPiano] – a keyboard, notes on a staff, and two common naming conventions. I will use letter names. The other representations are there to aid anyone familiar with them. You do not need to learn them to understand this chapter or to work the problems.
%If you have access to a keyboard or know how to play the notes on another instrument, you can produce some of the consonant and dissonant combinations yourself. However, since modern tuning is slightly different, you will not hear all of the effects. In this chapter we will look at two versions of the C major scale, Ptolemy's and the modern tuning. By the end of the chapter you should  understand what you can do on your modern instrument. If you can alter the tuning of your instrument, then you can experiment with the Ptolemaic scale as well.

Above the keyboard in figure \in[fig:TinyPiano] are lines showing the relative lengths that will produce the pitches. These lengths could be strings or pipes.
%For example, they could be one octave’s worth of strings in a grand piano, as in figure \in[fig:GrandPiano].
Galilei taught us that it is not the ratios of lengths that matter, but the ratios of frequencies, so the ratios in figure \in[fig:TinyPiano] are frequency multipliers.
%We already have frequencies for both Cs, F, and G, and the intervals connecting them in figure \in[fig:PythagoreanPipesOct]. The exact frequencies are not as important as the ratios
...
\stopexample
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
This horrible looking sequence of steps multiplies to make a beautiful perfect fifth! (Prime factorization makes these calculations a breeze. You may want to refresh your powers of two, three, and five. Every number in the Ptolemaic system is a product of twos, threes, and fives.)
...
The Ptolemaic scale does contain a couple of unwelcome surprises. The interval from D to A looks like a perfect fifth on the keyboard – three whole steps and one half step. However, adding the steps does not give the consonant \threehalves\ ratio, instead it gives a dissonant ratio:
\startformula
