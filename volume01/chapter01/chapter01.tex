...
Ancient Egyptians, Babylonians, Chinese, and others found ways to blend different pitches to produce the full, rich sounds that give music its expressive power.
...
\stopexample
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\startexample[ex:PyConsonance]
Pythagoras and Philolaus invite you select a pipe and join their demonstration. They will play pipes with lengths 8 and 12. You want to select a remaining pipe (length 4, 6, 9 or 16) that is consonant with both of their pitches. According to the Pythagorean model, which of their pipes should you avoid?
\startsolution
...
Performance instruments play many specific pitches determined by the lengths of their pipes or strings, or by the placement of holes or frets.
Ideally, many of these pitches can be combined in consonance, which means many of the pipes' or strings' lengths are related by simple ratios.
...
The separation between two pitches is called the \keyterm{interval} between the pitches. Pitches that are close together on the scale are separated by a small interval, while pitches that are farther apart are separated by a larger interval.
...
Ptolemy's scale contains many strings in consonant ratios of three-to-two or four-to-three. These consonant pairs are separated by fifths or fourths.
...
All of the consonant ratios from the Pythagorean model can be made from Ptolemy's octaves, perfect fifths, and perfect fourths. With so many of consonant pairs, musicians could blend consonant pitches starting from almost any note on the scale.
...
The scale's smallest intervals are individual steps, which are always dissonant. Ptolemy's scale has five larger steps and two smaller steps. The larger steps, with ratios of $9/8$ or $10/9$, are \keyterm{whole steps.}
The whole steps are obvious on the keyboard (\in{fig.}[fig:TinyPiano]) because there is always a black key in the middle of a whole step.
(The black keys are sharps and flats. There were many arguments about the exact ratios for the sharps and flats, which we will avoid.)
...
Each step in the scale is either a whole step or a half step.
Use \in{figure}[fig:TinyPiano] to find the two half steps (from E to F and from B to C) and the five whole steps.

Specific combinations of half steps and whole steps produce the consonant intervals in Ptolemy's scale. These consonant intervals are shown in \in{table}[T:PtIntervals], along with their ratios and their steps. For example, all thirds contain two steps. A major third contains two whole steps, while a minor third contains one whole step and one half step.

Ptolemy's experiments with consonant thirds forced a small but significant revision of the Pythagorean model of consonance. Ptolemy still believed that simple ratios produce consonant sounds, but simple ratios could contain numbers up to six, like the ratios $5/4$ and $6/5$.
Ptolemy's scale, with these additional consonances, was a tremendous gift for musicians, allowing them to create a huge variety of exciting sounds from an abundance of consonant pitches.
...
\FL[2]%\toprule
\NC Interval	\NC Ratio	\NC Steps					\NR
\HL
\NC Octave		\NC $2:1$	\NC $5$ whole + $2$ half	\NR
\NC Perfect 5th	\NC $3:2$	\NC $3$ whole + $1$ half	\NR
\NC Perfect 4th	\NC $4:3$	\NC $2$ whole + $1$ half	\NR
\NC Major 3rd	\NC $5:4$	\NC $2$ whole				\NR
\NC Minor 3rd	\NC $6:5$	\NC $1$ whole + $1$ half	\NR
\LL[2]%\bottomrule
\stoptabulate}
...
(Fortunately, most of \booktitle{Harmonics} survived, perhaps in the Islamic world.)
...
As Europe emerged into the Renaissance, musicians became more adventurous and began using thirds in their compositions. Music theorists reinvented the Ptolemaic scale, which became known as \keyterm{just intonation.} Then, part of the lost \booktitle{Harmonics} was found, adding Ptolemy’s authority to the system of just intonation. Just intonation was integrated into a sophisticated musical theory by the great Italian master Gioseffo Zarlino in \booktitle{Le Istitutioni harmoniche,} published in 1558.
...
It was embarrassing because many books on music theory, from ancient times right up until Galilei’s day, suggested that these experiments had been done and that the results supported the model.
...
To use this revised mathematical model of musical consonance, we must quantify frequency, which measures how quickly the instrument's vibrating motion is repeated.  Each repetition is called a \keyterm{cycle}, so frequency is measured in cycles per second.  The long, heavy strings in a grand piano vibrate with a frequency of a few dozen cycles per second, producing low pitches.  The shortest strings’ frequencies are thousands of cycles per second, producing high pitches.
The unit for frequency is \keyterm{hertz}, abbreviated as Hz.

To find consonances, we are no longer interested in the lengths of pipes or strings, only their frequencies.
The frequencies for just intonation are shown in \in{table}[tab:Frequencies].
(The table also shows Galilei's equal-tempered scale, which is the subject of the next section.)
These frequencies do not extend to the limits of human hearing, but they do cover a huge range\dash from the lowest A at $27\onehalf\units{Hz}$ up to the highest C at $4224\units{Hz}$. The middle A, used to tune an orchestra, is near the middle, at $440\units{Hz}$.

\startformula
1\units{Hz} = 1\units{cyc/s}
\stopformula
Humans can hear a huge range of frequencies, from about \unit{5 Hz} up to \unit{20,000 Hz}.  The highest frequencies are only audible to young people.

\startplacetable[location=margin, reference=tab:Frequencies, title={Frequencies (Hz) of Zarlino's just intonation scale and Galelei's equal-tempered scale.}] % Caption
\startMPcode
  pickup pencircle scaled 0.4pt ;
  string Note, NoteName[], FreqStr ;
  KeyboardWidth := 20cm ;
  WholeWidth := KeyboardWidth/52 ;
  HalfWidth := 7*WholeWidth/12 ;
%  KeyboardDrop := WholeWidth+HalfWidth ;
  path RightSquare, WhiteKey, BlackKey ;
  RightSquare := fullsquare  xshifted -0.5 ;
  WhiteKey := RightSquare  xyscaled (13mm,WholeWidth) ;
  BlackKey := RightSquare xyscaled (8mm,HalfWidth) ;
  JustColx := 8mm ;
  EqColx := 2.7cm ;
  i := 0 ;
  for Note="C","D","E","F","G","A","B","C":
  ...
  i := 0 ;
  for NoteFreqBase=24,27,30,32,36,40,45:
    NoteFreqJust[i] := (11/16)*NoteFreqBase ;  
    i := i+1 ;
  endfor
  i := 0 ;
  for HalfStep=-9,-7,-5,-4,-2,0,2:
    NoteFreqEq[i] := (440/16)*(2**(HalfStep/12)) ;  
    i := i+1 ;
  endfor
  i := 0 ;
  for HalfStep=-8,-6,-3,-1,1:
    AccidentHalfStepBase[i] := HalfStep ;  
    AccidentFreqEq[i] := (440/16)*(2**(HalfStep/12)) ;  
    i := i+1 ;
  endfor
    label.top  ("Note\strut", (-0.65cm,0)) ;  
    label.top  ("Just\strut", (JustColx,0)) ;  
    label.top  ("Equal-temp.\strut", (EqColx,0)) ;  
  for BlackKeyNum=0 upto 35:
    OctNum := (BlackKeyNum+4) div 5 ;
    AccidentNum := (BlackKeyNum+4) mod 5 ;
    FreqEq := AccidentFreqEq[AccidentNum]*(2**OctNum) ;
    % FreqEq := NoteFreqEq[NoteNum]*(2**OctNum) ;
    y := -((12*OctNum)+AccidentHalfStepBase[AccidentNum]+1)*HalfWidth ;
    fill BlackKey yshifted y ;
    if FreqEq<100:
      draw thefmttext.rt("$@.3f$",FreqEq) shifted (EqColx+1mm,y) ;
    elseif FreqEq<1000 :
      draw thefmttext.rt("$@.2f$",FreqEq) shifted (EqColx+1mm,y) ;
    else:
      draw thefmttext.rt("$@.1f$",FreqEq) shifted (EqColx+1mm,y) ;
    fi
    %label.rt  (OctNum, (-1cm,y)) ;  
    %label.rt  (AccidentNum, (-0.5cm,y)) ;  
  endfor
  for WhiteKeyNum=0 upto 51:
    y := -(WhiteKeyNum+0.5)*WholeWidth ;
    OctNum := (WhiteKeyNum+5) div 7 ;
    NoteNum := (WhiteKeyNum+5) mod 7 ;
    FreqJust := NoteFreqJust[NoteNum]*(2**OctNum) ;
    FreqEq := NoteFreqEq[NoteNum]*(2**OctNum) ;
    %FreqStr := thefmttext.lft("$@.1f$",FreqEq) ;
    draw WhiteKey yshifted y ;
    FreqJustInt := floor FreqJust ;
    FreqJustFrac := FreqJust - FreqJustInt ;
    draw thefmttext.lft("$@.0f$",FreqJustInt) shifted (JustColx+.8mm,y) ;
    if FreqJustFrac == 1/2:
      label.rt  ("$\onehalf$", (JustColx,y)) ;  
    elseif FreqJustFrac == 1/4:
      label.rt  ("$\onequarter$", (JustColx,y)) ;  
    elseif FreqJustFrac == 3/4:
      label.rt  ("$\threequarter$", (JustColx,y)) ;  
    elseif FreqJustFrac == 1/8:
      label.rt  ("$\oneeighth$", (JustColx,y)) ;  
    elseif FreqJustFrac == 7/8:
      label.rt  ("$\seveneighth$", (JustColx,y)) ;  
    elseif FreqJustFrac == 15/16:
      label.rt  ("$\fifteensixteenths$", (JustColx,y)) ;  
    fi
    if FreqEq<100:
      draw thefmttext.lft("$@.3f$",FreqEq) shifted (EqColx-1mm,y) ;
    elseif FreqEq<1000 :
      draw thefmttext.lft("$@.2f$",FreqEq) shifted (EqColx-1mm,y) ;
    else:
      draw thefmttext.lft("$@.1f$",FreqEq) shifted (EqColx-1mm,y) ;
    fi
    label  (NoteName[NoteNum], (-1.05cm,y)) ;  
%    label.rt  (WhiteKeyNum, (-1.5cm,y)) ;  
%    label.rt  (OctNum, (-1cm,y)) ;  
%    label.rt  (NoteNum, (-0.5cm,y)) ;  
  endfor
\stopMPcode
\stopplacetable

Zarlino's just intonation scale has all the wonderful consonances of Ptolomy's scale (\in{tab.}[T:PtIntervals]).
The frequencies of just intonation reveal another gem for musicians\dash consonant sets of three pitches called \keyterm{triads}. The most obvious consonant triad is the set C, E, and G. We can see the consonance by looking at the frequency ratio. (I will use frequencies from the middle octave, but any octave will work.)
\startformula
   264\units{Hz} : 330\units{Hz} : 396\units{Hz}
\stopformula
To simplify this ratio, divide by the common factor $66\units{Hz}$.
\startformula
   \frac{264\units{Hz}}{66\units{Hz}} : \frac{330\units{Hz}}{66\units{Hz}} : \frac{396\units{Hz}}{66\units{Hz}}
   = 4 : 5 : 6
\stopformula
A triad forming a consonant four-five-six ratio is called a \keyterm{major triad}
Major triads contain a major third ($4\mathord{:}5$), a minor third ($5\mathord{:}6$), and a perfect fifth ($4\mathord{:}6=2\mathord{:}3$). All of this consonance gives the major third its heroic sound.

The pitches A, C, and E form a \keyterm{minor triad}. Minor triads also contain and major third, a minor third, and a perfect fifth, but minor triads are not as consonant as major triads. You can use our model of musical consonance to see why\dash just calculate the three-note ratio.
...
Just intonation does contain a few unwelcome surprises. The interval from D to A looks like a perfect fifth on the keyboard – three whole steps and one half step. However, the ratio of frequencies is not a consonant \threehalves\ ratio, instead it is a dissonant ratio:
\startformula
	\frac{440\units{Hz}}{297\units{Hz}}
		= \frac{40}{27}.
\stopformula
This fifth is called the wolf fifth. If a composer puts this fifth in a piece, at the performance it will howl like a wolf! I will let you find the wolf fourth hiding in Zarlino's scale.
...
Following a suggestion of Aristoxenus, Galilei divided the octave into twelve identical half steps. Galilei calculated that each half step would change the frequency by a factor of $\sqrt[12]{2}$. This method, called \keyterm{equal temperament,} is shown in figure \in[fig:TinyPianoCompleteEq].
The frequencies of the keyboard's black keys (the sharps and flats) fit naturally into equal temperament.

Every ratio in the equal tempered scale can be calculated from the half step's ratio of $\sqrt[12]{2}$. For example, a whole step from C to D is two half steps. The first half step, from C to C\sharp, raises the frequency by a factor of $\sqrt[12]{2}$. The second half step, from C\sharp\ to D, raises the frequency by another factor of $\sqrt[12]{2}$. Therefore, the whole step from C to D raises the frequency by a factor of $\sqrt[12]{2}\times\sqrt[12]{2}$.
\startformula
	{\textstyle \sqrt[12]{2} \times \sqrt[12]{2}}
		= 2^{\onetwelfth} \times 2^{\onetwelfth}
		= 2^{\text{\fractwo\twelfths}}
		= 2^{\onesixth}
		= {\textstyle \sqrt[6]{2}}
\stopformula
A whole step raises the frequency by a factor $\sqrt[6]{2} \approx 1.059$, which is not a simple ratio.

\placefigure[margin][fig:TinyPianoCompleteEq]{The equal-tempered tuning is built from twelfth roots of two. String lengths in this ratio are shown above the keyboard.} {\externalfigure[TinyPianoCompleteEq][width=\rightmarginwidth]}
...
\startformula
	%\sqrt[12]{2} \times \sqrt[12]{2} \times \sqrt[12]{2} \times \sqrt[12]{2} \times \sqrt[12]{2} \times \sqrt[12]{2} \times \sqrt[12]{2} = 
	\left({\textstyle \sqrt[12]{2}}\right)^7
		= \left(2^{\onetwelfth}\right)^7
		= 2^{\seventwelfths}
		\approx 1.498
\stopformula
The equal-tempered perfect fifth is $2^{\seventwelfths}$. This is not a simple ratio, but it is very close to $3/2$, the ratio of a just perfect fifth. The equal-tempered perfect fifth sounds perfect to most listeners.
\stopsolution
\stopexample
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Equal temperament's success caused a final revision in the mathematic model of musical consonance: \emph{frequencies approximating simple ratios are consonant.} (Experiments show that ratios within about one percent of a simple ratio are good enough to sound consonant, although trained ears will notice the slight mismatch.)
The success of equal temperament eliminated perfect simple ratios from the musical scale, but it also represents a great triumph for the Pythagorean’s mathematical model. The equal-tempered tuning succeeds because it produces frequency ratios that are very close to Pythagoras and Ptolemy’s simple ratios.
...
If all the frequencies in \in{figure}[tab:Frequencies] were ten percent higher, the ratios would remain the same.
...
This will give the frequency in cycles per second, or hertz, the common unit of frequency.
...
Duration is found by subtracting the initial time from the final time. Using $t\si$ for the initial time and $t\sf$ for the final time, we can write a simple formula for the duration.  
\startformula
\Delta t = t\sf - t\si
\stopformula

The subscripts \quote{i} and \quote{f} will be used frequently to represent initial and final values.  Note that final time comes first in the formula, and the initial time comes last.  This order, final minus initial, always gives a positive duration.

	%%%%%%%%%%%%%%%% EXAMPLE 1.3 %%%%%%%%%%%%%%%%
\startexample[ex:frequency]
	To determine the frequency of a violin string, you record the plucking of the string with a high-speed video camera.  Watching it in slow motion, you see the string plucked at $3.25\units{s}$ in the video and count $55$ cycles before ending at $3.50\units{s}$.  What is the frequency of the string’s vibration?
...
Frequencies of other notes can be found by using the ratios of equal temperament.
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The Pythagorean model refined}
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Before we investigate the Pythagorean’s mathematical model of the cosmos, there is one thing that I want you to remember: this model is terrible! Celestial objects do not circle Earth. Only the Moon goes around Earth, and none of the objects circle, they travel along ellipses. These ellipses are nearly circular, so a model made from circles could be a good, approximate model the Solar System. 
However, placing Earth at the center of these circles produces a convoluted mess. This became clear almost immediately to anyone making regular observations of the planets.
...
\placetable
    [margin]
    [T:Astrosym]
    {Astronomical Symbols for some important objects in our Solar System. Most of these symbols can be found in figure \in[fig:Ptolemaicsystem]. The two outer planets, Uranus and Neptune, were discovered in 1781 and 1846, respectively.}
{\vskip330pt\noindent% \vskip330pt
...
Eratosthenes had moved to Egypt to be a librarian at the renowned Library of Alexandria.
Alexandria, on Egypt's northern coast, was perfect for Eratosthenes' measurement.
...
Either way, he found the distance to be about $5000$ stadia, or about \unit{900km}. Multiplying by $50$, he estimated Earth’s circumference to be about \unit{250,000} stadia, or about \unit{45,000 km}.
Earth’s circumference is now known to be \unit{40,000 km} (or \unit{4.00e7 m}). Eratosthenes’s estimate is about $12\%$ larger than the correct value, impressively accurate considering the difficulty of the measurement.
...
In the centuries after Eratosthenes, astronomical measurements improved dramatically. The Babylonians, in particular, became master astronomers. They had been recording daily observations of celestial motions since the seventh century \scaps{bc} and continued into the first century \scaps{ad}. This 800 year record was essential for Greek astronomers attempting to refine their model of the world.

When Ptolemy attacked this problem, armed with the precise astronomical observations of the Babylonians, he produced a geocentric model with dozens of complicated, invisible wheels that moved the planets through their complex motions. Unlike his elegant system of intonation, his model of the world is insanely complex and arbitrary. He explained all of the details in the \booktitle{Almagest,} which was not lost. His geocentric model, the \keyterm{Ptolemaic system,} became the accepted model of the world through the middle ages.


During the Renaissance, when musicians rediscovered consonant thirds, the Polish astronomer Nicholas Copernicus rediscovered the heliocentric model of the world. His model used the same Babylonian data as Ptolemy – still the best data in existence! All of the details are in his 1543 book, \booktitle{On the Revolutions of the Heavenly Spheres.} Figure \in[fig:CopernicanSystem] is Copernicus' simplified sketch of his system. His full model, like Ptolemy’s, was insanely complex, requiring dozens of spheres to account for all of the planets' motions. His heliocentric model is often called the \keyterm{Copernican system}.

Given the complexity of the Copernican system, few people were persuaded to abandon Ptolemy's geocentric model.  Finding the correct model of the world would require two things: new observations and a willingness to abandon failed ideas.
...
Then, in 1609, Galileo heard of a device which seized his attention completely. He tells the story of this device and the discoveries that followed in his 1610 booklet, \booktitle{The Sidereal Messenger}. (Sidereal means \quotation{of the stars.})
\startblockquote
...
Galileo shares his observations with delight and wonder, and he uses his observations to better understand the nature of heavenly bodies.

Galileo recorded his observations of the Moon in careful watercolors, \in{figure}[fig:MoonSurface], revealing details never before seen.
Using these details, Galileo challenges widely held beliefs about the Moon.
% Margin image
\placefigure[margin][fig:MoonSurface]{The Moon’s hills and valleys are clearly visible in Galileo’s original watercolors.\endnote{Galileiana 48, f.~20, Biblioteca Nazionale Centrale, Florence}} {\externalfigure[MoonPhases6][width=\rightmarginwidth]}
...
%\stopblockquote
\startblockquote
	...the surface of the moon is not perfectly smooth, free from inequalities and exactly spherical (as a large school of philosophers holds with regard to the moon and the other heavenly bodies), but\dots on the contrary it is full of inequalities, uneven, full of hollows and protuberances, just like the surface of the earth itself, which is varried everywhere by lofty mountains and deep valleys.\autocite{p.~52}{Galileo1610}
\stopblockquote
...
\noindent
The geocentric model's proponents believed that heavenly bodies are perfectly smooth and round, made of an immutable substance unlike anything on Earth. In their model, the visible patterns on the Moon are like the patterns on perfectly smooth, polished marble. Galileo, however, saw small details on the Moon that would change with the changing direction of sunlight. He carefully argues that each of these changes is exactly what is expected for the shadows and illuminated slopes of mountains, valleys, ridges and hollows. He compares each changing detail on the Moon to the changing shadows and illuminated slopes seen on Earth as the Sun rises and sets.
With each of these comparisons Galileo undermines the essential geocentric distinction between Earth and the heavens.

Galileo next shared his amazement when the telescope, \quotation{set distinctly before the eyes other stars in myriads, which have never been seen before, and which surpass the old, previously known, stars in number more than ten times.}\autocite{p.~48}{Galileo1610}
He drew a sample, figure \in[fig:Orion], demonstrating the telescope’s ability to reveal new details in the heavens.

Galileo used the telescope's power to resolve an ancient dispute about the nature of the Milky Way and other faint, nebulous illuminations in the night sky. Were these illuminations made of stars too faint and numerous to distinguish, or where they diffuse substances like clouds reflecting the light of other stars or of the Sun?

\startblockquote
The third thing which I have observed is the essence or substance of the Milky Way. By the aid of a spyglass anyone may behold this in a manner which so distinctly appeals to the senses that all the disputes which have tormented philosophers though so many ages are exploded at once by the indubitable evidence of our eyes, and we are freed from wordy disputes upon this subject. In fact, the galaxy is nothing but a mass of innumerable stars planted together in clusters. For upon whatever part of it you direct the spyglass, straightaway a vast crowd of stars presents itself to view; many of them are tolerably large and extremely bright, but the number of small ones is quite beyond determination.
\autocite{p.~66}{Galileo1610}
\stopblockquote

%\startblockquote
%Milky brightnesses, like the brightness of a white cloud, is seen not only in the Milky Way, but also in several spots of a similar color that shine here and there in the heavens. If you turn the spyglass upon any of them, you will find a cluster of stars packed close together. Furthermore\dash and you will be more surprised at this\dash the stars that have been called by every one of the astronomers up to this day \emph{nebulous} are groups of small stars set thick together in a wonderful way. Although each star escapes our sight on account of its smallness, or of it immense distance from us, from the commingling of their rays there arises that brightness which has hitherto been believed to be the denser part of the heavens, able to reflect the ray of the stars or the sun.
%\autocite{p.~66--67}{Galileo1610}
%\stopblockquote

Finally, Galileo relates his most amazing discovery.
\startblockquote
	There remains the matter that seems to me to deserve to be considered the most important in this work. That is, I should disclose and publish to the world the occasion of discovering and observing four planets never seen from the beginning of the world up to our own times, their positions, and the observations made during the last two months about their movements and their changes of magnitude [brightness].
\autocite{p.~67--68}{Galileo1610}
\stopblockquote

Galileo discovered these planets near Jupiter, observing three of them for the first time on January 7, 1610. Over the next few nights he observed them moving quickly around Jupiter, so that on some nights they appeared just to the east of Jupiter, and on other nights just to the west.
...
\stopblockquote
The four celestial objects that Galileo discovered, which he referred to as \quotation{planets,} \quotation{stars} and \quotation{wandering sidereal bodies} in the passages above, are now called moons of Jupiter, or jovian moons.
Today, the term \keyterm{star} is reserved for large, hot objects that produce their own light, like the Sun.
The term \keyterm{planet} is reserved for objects orbiting the Sun or another star.
\emph{Moon}\index{moon} refers to an object orbiting a planet. Jupiter has many moons too small for Galileo  to have seen with his telescope.
...
Galileo's observations convinced him completely that the heliocentric model is correct.
He clearly hopes to convince his readers as well! He promises a more complete argument\dash backed by more evidence\dash in an upcoming book, \booktitle{On the System of the World}.

\startblockquote
For I will prove that the earth has motion\dots and I will support these conclusions by countless arguments taken from natural phenomena. 
\autocite{p.~63}{Galileo1610}
\stopblockquote
Galileo will guide us through his most compelling arguments in \in{Chapter}[ch:Motion].
To understand his arguments, we must first learn a bit more about circular motion, which drive's Galileo's model of the world.

\section{Period relations}

When Galileo says that Jupiter completes an orbit \quotation{in the period of twelve years,} he is using \quotation{period} just as it was used for musical vibrations. Period\index{period} is still the time required for one \keyterm{cycle}.
...
The orbits of planets can be described by their frequencies rather than their periods. For example, Mercury orbits the Sun with a period $T=88.0$ days. The frequency can be found using the frequency relations. We could find the frequency in Hertz, but since the period is months long, let us find the frequency in cycles per year.
\startformula
...
A circular orbit’s radius is measured from the orbit’s center.
...
%\startblockquote
%And I summon all astronomers to apply themselves to examine and determine their periodic times, which it has not been permitted me to achieve up to this day owing to the restriction of my time.\autocite{p.~68}{Galileo1610}
%\stopblockquote

\stopsection

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\startsection[title={A revolution in astronomy}]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\booktitle{The Sidereal Messenger}, published in March of 1610, made Galileo a celebrity. In April, the grand duke of Tuscany, Cosimo II de' Medici, appointed Galileo to be his \quotation{Philosopher and Chief Mathematician.} Galileo quit his job at the University of Padua and moved to Florence where he refined his telescopes and studied the heavens. 
