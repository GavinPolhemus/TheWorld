...
Music’s precise, expressive nature caught the attention of many early civilizations. Ancient Egyptians, Babylonians, Chinese, and others found ways to blend different pitches to produces full, rich sounds that give music its expressive power. 
Many early civilizations also discovered that simple mathematical ratios predict which pitches will blend well.
...
But his followers, the Pythagoreans, did possess a simple mathematical model for predicting which pitches would blend together comfortably, and which would clash uncomfortably.
...
According the Pythagorean model, would pipes with lengths 12 and 8 produce consonant pitches?
\startsolution
...
\stopexample
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\startexample[ex:PyConsonance]
According the Pythagorean model, would pipes with lengths 16 and 9 produce consonant pitches?
\startsolution
The ratio sixteen-to-nine does not simplify and includes numbers bigger than four. These pipes produce dissonant pitches.
\stopsolution
\stopexample
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Before we look at these challenges and revisions, I will introduce another Pythagorean model\dash a mathematical model of the world.
...
Pythagoras and Philolaus's pipes (\in{fig.}[fig:Pythagoras]) would be ecellent for demonstrating consonance and disonance, but not useful for performing real music.
...
\noindent
Ptolemy, writing near the end of the classical period in the second century \scaps{bc}, took a more balanced view, seeking to satisfy both sense and reason. 
He performed experiments using an octachord, shown in \in{figure}[fig:TinyPiano]. This instrument has eight identical strings, all tuned to the same note. To produce other pitches. Ptolemy would place bridges under the strings. Each bridge reduces the length of string that can vibrate, raising the pitch. By adjusting each bridges' location, Ptolemy could easily experiment with many different combinations of lengths to determine which sound best.
...
Pythagoras was not committed to Pythagorean numerology, so he wished to include the consonant simple ratios of four-to-five and five-to-six. 
...
The separation between two pitches is called an \keyterm{interval}. Pitches that are close together on the keyboard (\in{fig.}[fig:TinyPiano]) are separated by a small interval, while pitches that are farther apart are separated by a larger interval.
...
What is the interval from E to B? According to the Ptolemaic model, are these pitches consonant?
...
\startsolution
The interval encompasses five pitches: E–F–G–A–B. Therefore, this interval is a fifth. To determine if it is consonant, we use the lengths in \in{figure}[fig:TinyPiano] to calculate the ratio.
\startformula
	\frac{48}{32}
		= \frac{3}{2}
\stopformula
The interval from E to B is a consonant fifth.
\stopsolution
\stopexample
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Fifths with the ratio $3/2$ are called \keyterm{perfect fifths}.
Fourths with the consonant ratio $4/3$ are \keyterm{perfect fourths}.
...
Each interval is associated with a ratio, as shown \in{table}[T:PtIntervals].
Octaves are always a ratio of 2.
...
\noindent
A modern grand piano spans over seven octaves, from the low A with a frequency $27.5\units{Hz}$ to the high C at $4186\units{Hz}$.
