...
Arrows show how the frequencies are related, with the ratio given as a frequency multiplier. The frequency where an arrow starts multiplied by the frequency multiplier is the frequency where the arrow ends. For example, the first pipe’s frequency multiplied by \m{4/3} is the second pipe’s frequency.
\startformula
...
Twelve is not the only number that works. Five is not bad, and is called a pentatonic scale. Nineteen actually works quite well and instruments using an equal tempered nineteen step scale were built and used in performance. One appears in Zarlino’s book (\in[fig:Zarlino19detail]). Twelve produces the best perfect fifth of any of these, and has never faced a serious threat from other equal tempered divisions of the octave.
