...
In Newton's collision example, the kinetic energy of ball $A$ would be
\startformula
...
Returning to Newton’s collision, all $A$'s kinetic energy after the inelastic collision can be easily found with the new kinetic energy formula.
\startformula
...
\startexample[ex:NewtonTotalInelastic]
	In Newton's collision example, one possible result is ball $B$ sticking to ball $A$ when they collide. In this case, what is their final velocity and how much kinetic energy is lost?

\startsolution
After the collision, since ball $B$ sticks to ball $A$, they can be treated as a single object with a total mass $M = m_A + m_B =4$. Momentum is conserved, and we already know that the total momentum before the collision is 16.
	\startformula\startmathalignment
		\NC p\si + \cancel{F\Delta t}	\NC = p\sf		\NR
		\NC 16					\NC = Mv\sf	\NR
		\NC v\sf					\NC = \frac{16}{M} = \frac{16}{4} = 4		\NR
	\stopmathalignment \stopformula
Even though the collision is totally inelastic, we were able to use conservation of momentum to find the final velocity $v\sf=4$. The resulting collision is shown in figures~\in[fig:NewtonTotalInelastic] and \in[fig:PendulumCollisionGraph5].
...
Since work is force times displacement, the {\sc si} units for work are $\unit{N\.m}$. These units are so common that they have been given their own name – \keyterm{joules}. The symbol for a joule is J (not to be confused with the symbol for impulse, $J$). % and it is related to the other standard SI units by the definition
\startformula
...
Finding the center of mass for a more complicated object is difficult, but always possible. Every object has a center of mass. Every object’s total momentum is $\vec p = m \vec v$ where $m$ is the object’s mass and $\vec v$ is the velocity of its center of mass. The total momentum always obeys conservation of momentum,
\startformula
	\vec p\si + \vec F\sn\Delta t = \vec p\sf,
\stopformula
where $\vec F\sn$ is the total of all external forces acting on any part of the object. The rotation can be totally ignored in the momentum calculation.
...
pic ["$\theta$", draw, angle eccentricity=1.4] {angle}
pic ["$s$", draw, angle radius=5cm, angle eccentricity=1.05] {angle};
\stoptikzpicture
\stopTEXpage
\stopbuffer

\placefigure[margin][fig:radians] % location
{The angle $\theta$ in radians is defined as ratio of the arc length $s$ to the radius $r$.}	% caption
...
Latitude and longitude are measured in degrees. Latitude goes from $0\degree$ at the equator to $90\unit{\degree N}$ at the North Pole (and $90\unit{\degree S}$ at the South Pole). Florence is $4870\units{km}$ north of the equator. What is Florence’s latitude? (Earth’s radius is $6370\units{km}$.)
\startsolution
...
pic [draw, <-, thick, angle eccentricity=1.2, "$\omega$", angle radius=12mm] {angle};
\stoptikzpicture
\stopTEXpage
\stopbuffer
...
pic [draw, <-, thick, angle eccentricity=1.2, "$\omega$", angle radius=8mm] {angle};
		\draw[thin] (0,0)--(5,0);
\stoptikzpicture
\stopTEXpage
\stopbuffer
...
\stopformula
As the disk rolls, the point in contact with the ground does not slide – it is the one point on the disk that is not moving. For an object of radius $R$ the center of mass velocity and angular velocity are linked by the rolling constraint:
\startformula
	v = R\omega.
\stopformula
