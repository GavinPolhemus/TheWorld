...
\startuseMPgraphic{graph::mynewgraph} % I'd like to add minor ticks, 0.667mm long.
vardef function =
	path p;
	for x = 0 step 1 until 10:
		y := 1.72-(0.2)*x; augment.p(x,y); endfor;
	p enddef;
draw begingraph(3.67cm,2cm);
	setrange(origin, 10,2);
	...
	gdraw(function) withpen pencircle scaled 0.8pt;
	glabel.urt("$H=K$",4);
	%glabel(fullcircle scaled 2cm, (0,0));
endgraph;
\stopuseMPgraphic

\startplacefigure[location=margin, reference=fig:BlockSlideK, title={The block’s energy as a function of position for \in{example}[ex:BlockSlide].}]
\small
\reuseMPgraphic{graph::mynewgraph}
\stopplacefigure
...
However, using $K = K\sub{CoM} + K\sub{rot}$, the center of mass kinetic energy $K\sub{CoM}$ and the kinetic energy due to rotation $K\sub{rot}$ are both independent of the orientation. We find them once, and they stay the same throughout the motion, confirming Du Châtelet's assertion that the \visviva\ always remains the same.
