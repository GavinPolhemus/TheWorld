...
\Initial{G}{ottfried Leibniz} was not satisfied with Newton’s momentum formula because it failed to achieve the basic goal of Descartes’s quantity of motion, which was to quantify the total motion of a system with many moving parts.
%To quantify the total motion in a multi-part system, Liebniz thought that the quantity of motion should be independent of direction. 
...
To understand Leibniz's concern, imagine two cannon balls resting harmlessly on the ground and two others fired directly toward each other at great speed (\in{figure}[fig:restingballs] and \in{}[fig:firedballs]).
Descartes and Newton would agree that the resting cannon balls have no motion (\in{table}[T:restingballs]).
They would disagree about the moving cannon balls (\in{table}[T:firedballs]).
Descartes’s quantity of motion, $m\abs{v}$, is positive for each moving cannon ball.
Therefore, according to Descartes, the total quantity of motion for the moving cannon balls is large. 
Newton, however, taught us to consider the direction of the motion.
Both moving cannon balls have momentum, $mv$, but one ball’s momentum is positive while the other ball’s momentum is negative. To Newton, the total momentum is \emph{zero} -- exactly the same as the total momentum of the two motionless cannon balls.
\placetable
    [margin]
    [T:firedballs]
...
Descartes’s quantity of motion is a reasonable guess, but collision experiments contradicted Descartes’s predictions.
Newton’s momentum gives correct predictions for collisions. But momentum fails to distinguish between the moving cannon balls' large motion and the resting cannon balls' lack of motion, giving zero in both cases. Leibniz wanted a quantity which would be positive for the moving cannon balls, but would also agree with experiments.

Like Newton, Leibniz sought inspiration in the laws of collision demonstrated with pendulum collisions.
Leibniz was especially impressed by Huygens, who had given a detailed analysis of these collisions.
\startblockquote
...
\stopblockquote
Contrary to Newton, Leibniz adopted Huygens' conserved quantity $mv^2$ as the replacement for Descartes’s quantity of motion. To understand why Leibniz liked Huygens’s $mv^2$, consider the cannon balls again.
Two cannon balls sitting in the field will have a total $mv^2$ equal to zero (\in{table}[T:restingballs]), but the two cannon balls fired straight at each other will have tremendous total $mv^2$ (\in{table}[T:firedballs]). Therefore, Huygens' experimentally verified $mv^2$ satisfied Leibniz's goals for a measure of total motion in a system.

Leibniz was so impressed with $mv^2$ that he called it the \visviva, or \quotation{living force,} responsible for motion. 
He elevated conservation of \visviva\ to the level of a universal law, claiming that it was conserved not only in elastic collisions, as Huygens had shown, but in all interactions.
...
Notice how Leibniz treats momentum as a directional quantity\dash positive for motion in one direction and negative for motion in the other, just as we have done.
\startblockquote
...
Notice in this passage, Leibniz did not suggest that momentum should be abandoned, but that the \visviva\ should also be considered.
To see how momentum and \visviva\ work together, look again at the specific collision Newton used as an example in his explanation of the conservation of momentum, shown in \in{figure}[fig:NewtonsExampleElastic].
In this one-dimensional collision, an object $A$ with a mass of $3$ and a velocity of $2$ was overtaken by a smaller object $B$ with a mass of $1$ and a speed of $10$.
...
Perhaps Leibniz’s \visviva\ can help us make a more specific prediction for this collision. First, we should calculate the initial \visviva\ in Newton’s collision. We are actually going to do something that is logically equivalent. We will calculate the closely related \keyterm{kinetic energy}.

\section{Kinetic energy}

Today, we do not use \visviva, but instead use \keyterm{kinetic energy}, which is half of the \visviva. The symbol for kinetic energy is $K$.
...
\stopformula
%}
The factor of one-half is of no consequence for Leibniz. If his \visviva\ quantifies the \quote{absolute} motion of the system, then one-half of the \visviva\ works just as well. Since we will use kinetic energy later, it is best to get in the habit of including the factor of one-half.
...
\stopformula
The two balls' total initial kinetic energy is $K\si=6+50=56$.
The initial momenta and kinetic energies for this collision are recorded in \in{table}[T:NewtonCollisionBefore].

To see if the momentum and \visviva\ are conserved, let us make a similar table showing the values after the collision. We will consider both of the outcomes mentioned above. First, consider the case with the larger momentum transfer, where ball $A$’s final momentum is $18$ and ball $B$’s is $-2$. Ball $A$’s final velocity is found from its momentum.
...
The effects of the inelastic collision can be seen by looking again at Newton’s example.
In the case where ball $B$ only transfers $4$ of momentum to ball $A$, both balls are a little more sluggish after the collision due to the loss of kinetic energy, as shown in figures~\in[fig:NewtonsExampleInelastic] and \in[fig:PendulumCollisionGraph4].
(Compare to the bounces in fig.~\in[fig:NewtonsExampleElastic] and fig.~\in[fig:PendulumCollisionGraph3].) 
The kinetic energy lost in the collision can be calculated by using the balls’ final momenta to find their velocities and then their kinetic energies.
The intermediate step of finding the velocity is tedious, so it is worth finding the direct relationship between kinetic energy and momentum.
...
\stopformula
Confirm the remaining kinetic energies in \in{table}[T:NewtonCollisionAfter1].
After the inelastic collision, the total kinetic energy is only 38, less than the 56 that the balls had before the collision. %Some of the energy has been \quotation{absorbed by the small parts which compose the mass} according to Leibniz.
\Visviva, which is twice the kinetic energy, is also lost, dropping from 112 before the inelastic collision to only 76 after.
However, notice that none of the momentum is lost.
In an inelastic collision, less momentum is transferred, but the total momentum stays the same.
Inelastic collisions lose kinetic energy (and \visviva) but not momentum!

The loss of \visviva\ in inelastic collisions posed a mortal threat to Leibniz’s conservation of \visviva. Leibniz knew that Descartes’s quantity of motion was abandoned due to similar experimental failures. Like Descartes, Leibniz defended his theory anyway, arguing that \visviva\ is conserved even in experiments where it appears to be lost. Leibniz did not invoke the unseen substances of Descartes, but instead sought to save \visviva\ within the objects themselves.
\startblockquote
...
\startformula
	K\sf	= \frac{p\sf^2}{2M}
		...
		= 32
\stopformula
The final kinetic energy is much less than the initial kinetic energy of 56. The kinetic energy lost is $K\si - K\sf = 56 - 32 = 24$. According to Leibniz, this kinetic energy is not lost to the universe but has gone into the balls’ minute parts.
\stopsolution
...
Leibniz recognized the truth of Newton’s momentum formula, even if he preferred his \visviva\ as an absolute measure of motion. His criticism of Descartes’s quantity of motion provoked a forceful defense from the Cartesians, who mistakenly pointed to Newton’s successful momentum as vindication. Over the next decade the \visviva\ debate became entangled with a bitter priority dispute between Newton and Leibniz over who had discovered calculus. Newton’s followers frequently viewed these debates as a question of loyalty rather than science. In England, where Newton was especially revered, \visviva\ was viewed with contempt.

On the continent, some scientists did not take sides purely out of loyalty, but instead approached the question with rigorous calculations and careful experiments, as scientists should. 
Johann Bernoulli, \pagereference[JBernoulli] a gifted young mathematician in Basel, Switzerland, began corresponding with the much older Leibniz in 1693.
Bernoulli was one of the first mathematicians to master the new calculus.
He rose to become professor of mathematics at the University of Basel, and was eventually recognized as one of the greatest mathematicians in Europe.
Leibniz's mathematical arguments persuaded Bernoulli to become a dedicated and vocal advocate for Leibniz and \visviva.
...
The Dutch scientist Willem ’s Gravesande was famous in Leiden, Holland for his lecture demonstrations. He published  excellent textbooks on Newtonian physics, the first appearing in 1720. These are filled with detailed woodcuts of his elegant experiments, like the pendulum collision apparatus on \at{p.}[fig:GravesandeCollider]. ('s Gravesande's diagrams directly inspired many experiments which I perform with my own physics students.)

To test the competing quantities of motion, 's Gravesande designed experiments to compare impacts of different objects traveling at different speeds.
He did this first by dropping balls into clay.
He worked with a set of balls identical in size but different in mass, shown in figures \in[fig:GravesandeDrop] and \in[fig:GravesandeDropSpheres].
A dropped ball would land with an inelastic thud, leaving a dent in the clay.
’s Gravesande measured the size of the dent  to determine the impact’s effect.
If a light, fast ball left the same size dent as a heavy, slow ball, then the two balls produced the same effect.
He thought the effect would be determined by the ball’s momentum $mv$, so that if the heavy ball’s mass is double the light ball’s mass, then the light ball’s velocity would have to be double the heavy ball’s velocity to make the same dent.
If the heavy ball was four times the mass of the light ball, then the light ball would need to be traveling four times as fast to produce the same effect.
...
While ’s Gravesande’s evidence won a few converts, many scientists remained loyal to the Cartesian quantity of motion, largely out of devotion to Newton. In 1728, French scientist Dortous de Mairan made a case for the Cartesian formula in his discourses on the \quote{Estimation and the Measure of Motor Forces of Bodies.} His forceful argument convinced most scientists on the continent that \visviva\ was a failed idea.\pagereference[deMairan]

De Mairan turned his attention elsewhere, studying the day-night cycles in certain plants.
However, outside the academic science community, the \visviva\ concept was picking up steam.
Now, we will turn our attention to the engineers. Later, we will return to de Mairan – with a humiliating surprise.
...
Lifting a heavy box or pushing a heavy cart requires work.
In all cases, work requires a force $F$ exerted over a displacement $\Delta x$.
The resulting amount of work $W$ is simply the product of the force and the displacement.
\startformula
	W = F\Delta x
\stopformula
%
...
Since work is force times displacement, the \scaps{si} units for work are $\unit{N m}$. These common units have their own name: \keyterm{joules}.
The symbol for a joule is J (not to be confused with the symbol for impulse, $J$).
\startformula
...
\section[sec:ConservationofEnergy]{Conservation of energy}

Engineers’ work and physicists’ \visviva\ were first linked by Daniel Bernoulli, son of the Swiss \visviva\ advocate Johann Bernoulli (\at{p.}[JBernoulli]).
D.~Bernoulli studied medicine and anatomy at the universities of Basel, Heidelberg, and Strasbourg, but he also learned mathematics from his father and older brother, Nicolaus {\sc ii} Bernoulli.
In 1725, Daniel and Nicolaus, who were great friends as well as brothers, accepted a joint invitation from Peter the Great of Russia to join the new St.~Petersburg Academy.
Months after their arrival, Nicolaus caught a fever and died.
He was replaced by Leonard Euler, another young student of Johann and friend of both Daniel and Nicolaus.

In St.~Petersburg, Daniel undertook a detailed study of fluid motion, which lead him to recognize that \visviva\ (or kinetic energy) is not the only type of energy. His analysis produced many surprising predications, which he verified with experiments. In 1729, he began writing a book, \booktitle{Hydrodynamica}, about his findings. With the book only partially complete, and frustrated by censorship and disputes about his salary, D.~Bernoulli left St.~Petersburg Academy and returned to the University of Basel in 1733. His letters to Euler in the following years reported slow progress until the eventual publication of \booktitle{Hydrodynamica} in 1738.

D.~Bernoulli used his conservation of energy principle to great effect in his groundbreaking  \booktitle{Hydrodynamica}. I will not introduce other types of energy now.
(D.~Bernoulli will assist us with that in the next chapter.)
For this chapter, simply know that there are other types of energy and only the total energy is conserved.
...
Energy can also be added as heat $Q$ by cooking the cart over a fire, which increases the random, microscopic energy of the cart’s atoms and molecules.
D. Bernoulli had some deep insights into how $Q$ related to the motion of the microscopic parts.
We will return to this topic in Volume II.
Until then, we will faithfully include $Q$ when writing the conservation of energy equation, but immediately cross it out because the heat transfer $Q$ is zero in mechanics problems.
In mechanics, any change in a system’s total energy is due to outside forces doing work on the system.
...
With any conservation law we must take care to define the system. With momentum conservation, we could choose which objects to include in the system. For example, in pendulum collisions we typically included both balls in our system, adding their momenta to find the system's total momentum. With conservation of energy we choose both the objects and the types of energy we wish to include in the system. In the pendulum examples we included the balls' kinetic energy, but not the energy of microscopic random motion. As we study more types of energy, we will carefully choose which types to include in the system. For the remainder of this chapter, only kinetic energy $K$ will be included in the system's total energy $H$. \pagereference[secend:ConservationofEnergy]
...
\startformula
	v = \sqrt{\frac{2(54\units{kg\.m^2/s^2})}{75\units{kg}}}
		...
		= 1.2\units{m/s}
\stopformula
Your work gives the cart $54\units{J}$ of kinetic energy and a speed of $1.2\units{m/s}$.
...
This is an appropriate place for me to make a frank confession: I have a terrible memory. I cannot remember these energy unit conversions.
However, I do remember the work formula and the kinetic energy formula.
They are short, and I have used each about one million times.
Whenever I need energy unit conversions, I just remember the formulas $W=F\Delta x$ and $K=\onehalf mv^2$. 
The left sides have the energy unit $\units{J}$ while the right sides have the units $\units{N\.m}$ and $\units{kg\.m^2/s^2}$, respectively
When I am working with non-{\sc si} units, as I often do, these formulas give me the conversions in whatever crazy units I am using.
Tricks like this made physics attractive to me at an early age!
(A professor at The University of Chicago, where I earned my Ph.D., observed that I passed the physics department's grueling candidacy exam without actually knowing much of anything. I took this as a vindication of my figure-it-out survival strategy.) 
...
The $75\units{kg}$ cart from \in{example}[ex:CartWork1] is coasting along the hallway with $54\units{J}$ of kinetic energy. As you approach your destination, you begin pulling backward on the cart to slow it. You pull with a backward force of $18\units{N}$. How far does the cart roll before coming to a stop?
...
draw begingraph(4cm,3.5cm);
	...
	glabel.top("$H=K$",1.6);
	glabel.bot("$F=\unit{27 N}$",0.5) rotatedaround(point 0.5 of H, angle (1,2.7));
	glabel.bot("$F=\unit{0 N}$",1.5);
	glabel.bot("$F=\unit{-18 N}$",2.5) rotatedaround(point 2.5 of H, angle (1,-1.8));
	glabel(myarrow, 0.5) rotatedaround(point 0.5 of H, angle (1,2.7));
	glabel(myarrow, 1.5);
	glabel(myarrow, 2.5) rotatedaround(point 2.5 of H, angle (1,-1.8));
endgraph;
\stopuseMPgraphic

\startplacefigure[location=margin, reference=fig:CartWork, title={The cart's energy graph for \in{examples}[ex:CartWork1] and \in[ex:CartWork2]. The graph's slope is the force applied to the cart.}]
\small
...
draw begingraph(4cm,1.5cm);
	...
	gdraw(H) withpen pencircle scaled 0.8pt;
	glabel.top("$H=K$",1.4);
	glabel.bot("$\unit{-9 N}$",0.6) rotatedaround(point 0.6 of H, angle (1,-0.9));
	glabel.bot("$F=\unit{0 N}$",1.5);
	glabel.bot("$\unit{18 N}$",2.4) rotatedaround(point 2.4 of H, angle (1,1.8));
...
\stopuseMPgraphic
\startplacefigure[location=margin, reference=fig:CartGraphs, title={The cart's energy graph as the empty cart is returned to $x=0$. This graph is drawn to the same scale as \in{figures}[fig:CartWork].}]
\small
\reuseMPgraphic{graph::CartGraphs}
\stopplacefigure

Initially, the empty cart is stationary at $x=8\units{m}$. By applying a gentle force in the negative direction ($F=-9N$), you push the cart $2\units{m}$ in the negative direction ($\Delta x = -2\units{m}$). By exerting a negative force through a negative displacement, you do positive work on the cart, increasing the cart's kinetic energy increases to $K = 18\units{J}$. 
...
The empty cart's kinetic energy is much less than the full cart's $54\units{J}$ of energy during your earlier trip.
I calculated the empty cart's speed using $K= \onehalf mv^2$ and found $\abs{v}= 2\units{m/s}$.
The empty cart's speed is greater than the heavy cart's, because the empty cart's mass is so much smaller. The empty cart's velocity is $-2\units{m/s}$ because it is moving in the negative direction. 
...
When the ball hits the clay, the clay exerts an upward force much greater than the gravitational force. The ball continues down a bit, making a dent in the clay, so there is a small negative (downward) displacement even as the clay is exerting a large positive (upward) force. The work done on the ball as it stops is the product of the positive force and the negative displacement. This negative work eliminates the ball's kinetic energy, brining it to a stop.
...
One idea, the conservation of \visviva, did gain rapid acceptance – but not initially due to a Bernoulli's work.
...
In 1740, at the age of thirty four, Du Châtelet wrote a physics book, \booktitle{Institutions de Physique} or \booktitle{Foundations of Physics}.
She had been unable to find an introductory physics text for her ten-year-old son, so she wrote \booktitle{Foundations} for him. She explained Newtonian mechanics with great clarity and conviction. No reader could doubt her mastery and enthusiasm for Newton’s theory of motion.

In the final chapter, Du Châtelet explains \visviva. She reminds her son of \quotation{the memoir that M.~de Mairan gave in 1728 to the Academy against \visviva\ that we read together, and in which the famous proceeding is explained with much clarity and eloquence.}
(You should also recall M.~de Mairan from \at{p.}[deMairan].)
She continues,
\startblockquote
...
\stopblockquote
Even as she refutes the Cartesian position, her mastery of Newtonian physics is on full display. In many  examples, she first uses Newtonian physics to predict what will happen. She then shows, based on this Newtonian analysis, that \visviva\ will also be conserved, but Descartes’ quantity of motion will not.
...
\stopformula
The kinetic energy is the same for both orientations, $K=2\units{J}$. With some additional work it is possible to show that the energy remains constant for all orientations.
\stopsolution
\stopexample
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Du Châtelet computed the \visviva, which is double the kinetic energy, and reached the same conclusion.
\startblockquote
It is easy to prove that the \visviva\ always remains the same, although the quantity of motion varies.\autocite{188}{DuChatelet1740}\pagereference[GlobesStop]
\stopblockquote
...
\stopblockquote
To see why the kinetic energy stays invariable in all cases, we must understand the kinetic energy of rotation.
...
\stopmathalignment\stopformula
where $I = \left(m\sub{A}^{}r\sub{A}^2 + m\sub{B}^{}r\sub{B}^2\right)$. Next we calculate $I$ and put that value in the simple formula $K\sub{rot}= \half I\omega^2$ above.
\startformula\startmathalignment
\NC	I	\NC = m\sub{A}^{}r\sub{A}^2 + m\sub{B}^{}r\sub{B}^2						\NR
\NC 		\NC = (3.0\units{kg})(0.25\units{m})^2 + (1.0\units{kg^2})(0.75\units{m})^2	\NR
\NC 		\NC = 0.75\units{kg\.m^2}										\NR
\stopmathalignment\stopformula
Using this in the formula for rotational kinetic energy gives
\startformula
\NC	K\sub{rot}	\NC = \half I\omega^2						\NR
\NC			\NC = \half (0.75\units{kg\.m^2})(2.0\units{r/s})^2	\NR
...
\startformula\pagereference[eq.Krot]
...
Any velocities, any masses, and any rigid shape will have a constant kinetic energy unless external forces do work on the object to change its energy.
Du Châtelet knew these complicated cases worked as well, but Newton's simple example made her point quite clearly (\at{p.}[GlobesStart]).
%(\at{pp.}[GlobesStart]–\at[GlobesStop]).

In addition to the Newtonian arguments, Du Châtelet provided further evidence for momentum and \visviva\ – and evidence against Descartes’ quantity of motion – by citing specific experiments like ’s~Gravesande’s. Du Châtelet understood the difference between Newton’s successful momentum and Descartes’ failed quantity of motion, and she drove a wedge between the two as she advanced the argument for \visviva.
...
Du Châtelet composed her detailed response in only three weeks — with no help from König! – demonstrating her brilliance and thereby removing any doubt that she was the true author of \booktitle{Foundations of Physics}.
...
Du Châtelet was working after midnight when she went into labor.
The birth was relatively easy.
She retired to bed for a few days but continued to work, completing corrections on the printer's proofs and then sending all of her working papers to the royal librarian.
Then, on the sixth day, she developed a sudden headache and had trouble breathing.
Perhaps a blood clot, formed during her time in bed, had broken free and lodged in her lungs.
The royal physician and the most prominent doctors of the region were called, but they could offer only opiates for some relief. She died that night, September 10, 1749.

Du Chatlâtet's translation and comentary helped advance physics on the continent, but it was largely ignored in England where the universities stubbornly continued using and teaching Newton's cumbersome geometric methods.

On the continent, energy continued to play a central role in the advancing physics, as we will see, and it continues to be central today. High-energy experiments done in the twentieth century revealed that the formula $K=\onehalf mv^2$ is not correct for speeds close to the speed of light ($3\sci{8}\units{m/s}$), just like the formula for momentum. Also like the formula for momentum, this kinetic energy formula is \emph{extremely} accurate for lower speeds (anything less than $10^7\units{m/s}$). For higher speeds we will need the updated formulas in Volume II.

Conservation of energy in its many forms is a bit more complicated than Leibniz’s original idea for conservation of \visviva, but added complexity also makes energy extremely useful. Food, fuel, and batteries all contain energy stored in other forms that can be converted into kinetic energy.
