...
Use D.~Bernoulli's new formula for gravitational potential energy whenever the problem involves objects far from Earth's surface. The new formula is more complicated looking than the old $U=mgh$, but use the new formula in exactly the same way when using conservation of energy.
...
For Lagrange's complex calculations, he wanted the small object's mass to drop out of the formulas earlier, before the formulas became too complicated.
...
\startplacefigure[location=margin, reference=fig:PottersThrow, title={A collision between a big piece of clay and the potter's wheel can be treated using conservation of angular momentum.}]
