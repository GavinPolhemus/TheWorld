...
Luckily, we can understand gravity's tremendous reach without mastering Einstein's full theory. All we  need is one part of the theory\dash a simple function called the \keyterm{gravitational potential}, $\Phi$.
...
The change in gravitational potential is $\Delta\Phi = \Phi\sf - \Phi\si$.
	...
	\stopmathalignment\stopformula
The final speed $v\sf$ depends only on the initial speed $v\si$ and the change in gravitational potential $\Delta\Phi$. Halley's Comet orbits the Sun, so the mass $M$ is the Sun's mass $M_\Sun = \unit{1.99e30kg}$. Use $GM_\Sun = (\unit{6.67e-11m^3/kg s^2})(\unit{1.99e30kg}) = \unit{1.33e20 m^3/s^2}$ in calculating the potential change.
	\startformula\startmathalignment
