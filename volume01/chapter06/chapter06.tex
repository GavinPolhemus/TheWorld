...
For Lagrange's complex calculations, he wanted the small object's mass drop out of the formulas earlier, before they became too complicated.
...
Next we calculate $\Delta \Phi$ from $\Phi = GM/r$, where $M$ is the Sun's mass $M_\Sun = \unit{1.99e30kg}$. $GM_\Sun = (\unit{6.67e-11m^3/kg s^2})(\unit{1.99e30kg}) = \unit{1.33e20 m^3/s^2}$.
	\startformula\startmathalignment
	\NC	\Delta\Phi	\NC = \Phi\sf	- \Phi\si	\NR
	\NC				\NC = -G\frac{M_\Sun}{r\sf} + G\frac{M_\Sun}{r\si}		\NR
	\NC				\NC = -GM_\Sun\left(\frac{1}{r\sf} - \frac{1}{r\si}\right)		\NR
...
While Lagrange wrote papers on celestial mechanics and won awards, he also pursued a much more ambitious goal.
...
Lagrange's equations of motion are a bit more tangled than our position and momentum update formulas, so we will not learn the details of his procedure. A general method for producing position and momentum update formulas was developed later by William Rowan Hamilton. We will need Hamilton's general method to solve the ancient problem of the planets' motions. We will introduce his method later in this chapter to study rotational motions. Then we will apply the method to the planets' elliptical orbits in \in{Chapter}[ch:Rotation]. 

First, however, we will look at circular orbits, which we can explain entirely with Lagrange's insights.

\section{Gravitational Field}
Newton's Law of Universal Gravitation, presented in the previous section, is a bit intimidating. It is certainly more complicated than the formula $F = -mg$, which we have been using for gravity near Earth's surface. The simpler formula describes the downward force in terms of the \keyterm{gravitational field} acting on the mass $m$. The gravitational field's direction is always downward. The gravitational field's magnitude is $g$. Near Earth's surface, $g$ is $\unit{9.8 m/s^2}$. At other locations in the solar system, $g$ can be much larger or much smaller.
We will continue to use the simple formula $F=-mg$, but we will take care to find the correct value of $g$.

The gravitational field's magnitude $g$ can be found at any distance $r$ from the center of any large mass $M$.
\startformula
  g = G\frac{M}{r^2}
\stopformula
Near Earth's surface, this formula gives the well known value for $g$.
\startformula
  g = G\frac{M}{r^2}
    = (\unit{6.67e-11 m^3/kg s^2})\frac{\unit{5.97e24 kg}}{(\unit{6.37e6 m})^2}
    = \unit{9.8 m/s^2}
\stopformula
...
\startformula
  F = -G\frac{mM}{r^2}
    = -m\left(G\frac{M}{r^2}\right)
    = -mg
\stopformula
In the last step we have use a new formula for \keyterm{gravitational field}.
\startformula
  g = G\frac{M}{r^2}
\stopformula
...
We now have two formulas for the downward gravitational force: Newton's Law of Universal Gravitation, presented in the previous section, and $F=-mg$, which we have used for gravity near Earth's surface (\at{p.}[eq:Fmg]). Both of these are correct, but 
...
\startformula
  \frac{v^2}{R} = \omega^2 R = g
\stopformula

Celestial motion was a primary interest for both Lagrange and Hamilton. They each hoped to solve puzzling celestial motion problems using their new, general, and powerful methods. Their methods start with energy, so we begin by looking at the energy graph for a planet orbiting the Sun, shown in \in{figure}[fig:PhiTerrestrial3D]. 

The Sun ($\Sun$) is at the center of \in{figure}[fig:PhiTerrestrial3D]. The planets are shown at increasing distances along the $r$ axis. (Planet symbols are listed in \in{table}[T:Astrosym] on \at{p.}[T:Astrosym].)
\in{Figure}[fig:PhiTerrestrial3D] is three dimensional to show the gravitational potential $\Phi$ as planets orbit the Sun. The vertical axis in \in{figure}[fig:PhiTerrestrial3D] is gravitational potential. The planets orbit in the vertical plane at the top of \in{figure}[fig:PhiTerrestrial3D].
...
Following the logic of Lagrange's insight, we can see from \in{figure}[fig:PhiTerrestrial3D] that the direction of the gravitatinal force is in the direction of the Sun, toward lower potential energy. The force toward the central Sun does not produce any torque $\tau$ on the planets, so each plant's angular momentum $L$ is constant.

\in{Figure}[fig:PhiTerrestrial3D] shows the planets' gravitational potential energy $U$, which is very negative near the Sun and rises toward zero far away.
\startformula
	U = -G\frac{mM}{r}
\stopformula
...
$r$ component of the gravitational force is negative, pulling planets toward lower potential energy in the direction of the Sun. This gravitational force toward the Sun bends the planets' paths into orbits.
...
\startformula
	F %= -\pp{r} U
		...
		= -G\frac{mM}{r^2}
\stopformula
The \emph{magnitude} of this gravitational force is the centripetal force above.
Setting the centripetal force $F= m\omega^2 r$ equal to Newton's gravitational force allows us to find the planet's angular velocity.
...
\stopmathalignment\stopformula
The planet's distance $r$ determines its angular velocity, with much higher angular velocities closer to the Sun. Higher angular velocities give shorter periods, which we find using $T = 2\pi / \omega$.
\startformula
...
Slowly rotating air masses become hurricanes or tornados when the air is drawn towards the center by an area of extreme low pressure.
