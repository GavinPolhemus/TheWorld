...
\stopformula
Remember that $\partial U$ is the small virtual change in the potential energy $U$ due to the small virtual displacement $\partial x$. If there are more coordinates, like $y$ for the cannon ball, then there are more direction for the virtual displacement. Lagrange found that he could get each component of Newton's force by choosing the correct direction for the virtual displacement. He found the $x$-component of Newton's force with a virtual displacement in the $x$-direction, the $y$-component with a virtual displacement in the $y$-direction, and so on for all the coordinates of the system. (These virtual displacements do not actually happen in the physical system, so Lagrange was free to imagine different virtual displacements to get the force's different components.)

For the cannon ball, a small virtual displacement $\partial x$ in the $x$-direction is a horizontal displacement, which does not change the cannon ball's potential energy at all. For this horizontal displacement, $\partial U = 0$, so
\startformula
	F_{\text{Newton,}x} = -\frac{\partial U}{\partial x}
		= -\frac{0}{\partial x}
		= 0
\stopformula
The force's $x$-components is zero, as we knew. A small virtual displacement $\partial y$ in the $y$-direction is a vertical displacement which does change the cannon ball's potential energy. For this vertical displacement, $\partial U = mg\partial y$, so
\startformula
	F_{\text{Newton,}y} = -\frac{\partial U}{\partial y}
		= -\frac{mg\partial y}{\partial y}
		= -mg
\stopformula
The force's $y$-component is $-mg$, again agreeing with what we knew.
...
\stopblockquote
 ...
 Lagrange’s work is so abstract that the book contains no diagrams and no physical arguments, only pure mathematics of idealized, general mechanical systems. For those fluent in the new mathematical methods of French analysis, Lagrange’s approach to mechanics is profound and beautiful. I will not attempt to explain a single word of it here.
