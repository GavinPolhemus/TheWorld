...
Next we calculate $\Delta \Phi$ from $\Phi = GM/r$, where $M$ is the Sun's mass $M_\Sun = \unit{1.99e30kg}$. $GM_\Sun = (\unit{6.67e-11m^3/kg s^2})(\unit{1.99e30kg}) = \unit{1.33e20 m^3/s^2}$.
	...
	\NC				\NC = -\frac{GM_\Sun}{r\sf} + \frac{GM_\Sun}{r\si}		\NR
...
\startformula
  g = \frac{GM}{r^2}.
\stopformula
We can use this formula to calculate $g$ here on Earth's surface. On Earth's surface, we are a distance $r = \unit{6.37e6 m}$ from Earth's center. Earth's mass is $M = \unit{5.97e24 kg}$. With these values, we find the well known value for $g$ near Earth's surface.
\startformula
  g = \frac{GM}{r^2}
...
\startformula
	F = \frac{pv}{r}
	  = \frac{mv^2}{r}
\stopformula
In this formula, $r$ the radius of the Venus's circular orbit and $v$ is Venus's speed. In the last step, Venus's momentum has been replaced with $p=mv$, where $m$ is Venus's mass.
...
\startformula\startmathalignment
\NC	\cancel{m}g \NC= \frac{\cancel{m}v^2}{r} \NR
%\NC	g \NC= \frac{v^2}{r} \NR
\NC	v \NC= \sqrt{gr}
    = \sqrt{\frac{GM}{r^2}r}
    = \sqrt{\frac{GM}{r}} \NR
\stopmathalignment\stopformula
In the last steps, we have replaced $g$ with the Sun's gravitational field, $g = GM/r^2$.
The speed $v$ is faster for smaller orbits' and slower for larger orbits. Not only do farther planets have to complete larger orbits with each cycle, they also travel along those orbits at a slower speed, making the periods of larger orbits much longer than the periods of smaller orbits.
%Planets close to the Sun travel their smaller orbits at greater speed. 

An orbit's period is its circumference divided by the planet's speed.
\startformula\pagereference[eq:Kepler3rd]
  T = \frac{2\pi r}{v}
	 = \textfrac{2\pi}{\sqrt{GM}}\,r^{\threehalves}
\stopformula
This is Kepler's celebrated third law, relating each planet's period $T$ to the orbit's radius $r$. A smaller radius produces a shorter period, while a larger radius produces a longer period.
...
The planet's mass has no effect on its period. 
%The Sun's strong gravitational pull on these inner planets bends their paths into small, fast orbits.
