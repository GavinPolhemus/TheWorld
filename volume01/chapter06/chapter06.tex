...
Lagrange's inspired insight was that Newton's force can be found from the energy graph of D.~Bernoulli's  potential energy. The force at a specific location is equal to \emph{minus} the slope of the potential energy at that location.
...
\startformula
	F\sub{Newton} = -\ppx U
\stopformula
The derivative in Lagrange's force formulas calculated in exactly the same way as the derivatives in the last section.
After some examples, I will explain why each little $d$ has been replaced by a little $\delta$. This change does not affect the calculations, so ignore it for now.
The important difference is the little $x$ in the derivative. This $x$ has replaced the $t$ because we are now finding the slope of the energy graph, which has $x$ as the horizontal axis.
