...
The Hamiltonian is the pendulum's total energy. When working with the the angle it is convenient to set the pivot at the height $h=0$, making the pendulum's height negative whenever it is below the pivot, as shown in \in{figure}[fig:GalileoPendulumAngle]. The pendulum's potential energy is then $U = -mgR\cos(\theta)$. The pendulum's rotational kinetic energy (\at{p.}[eq.Krot]) is $K = \half I\omega^2$, where the moment of inertial $I$ plays the role of rotational mass. Unfortunately, this is written in terms of angular velocity $\omega$. The Hamiltonian must be written in terms of angular momentum $p_\theta$. The obvious guess is $K = p_\theta^2/2I$, where $I$ again plays the role of rotational mass. This guess is correct, and the Hamiltonian is
\startformula
...
Now that we have the Hamiltonian, we can use Hamiltons equations to find the equations of motion. These will be the update formulas for $\theta$ and $p_\theta$.
\startformula
...
\stopformula
This is the usual position update formula for angular motion $\theta = \omega\,dt$ with angular velocity $\omega = p_\theta / I$.
The angular momentum $p_\theta$ is related to the angular velocity in the same way that Newton's momentum is related to velocity.
\startformula
	p_\theta = I\omega
\stopformula
...
\stopformula
Completing this calculation requires one calculus fact: $\ppth \cos(\theta) = -\sin(\theta)$. (In a calculus course you would learn the derivatives of many special functions. You do not need to learn them here.) With this fact we can finish the calculation.
\startformula
...
%\stopmathalignment\stopformula
This is the usual momentum update formula $p_\theta = F_\theta\,dt$ with the angular force $F_\theta = -mgR\sin(\theta)$. The angular force is usually called \keyterm{torque}.%, and is often represented by $\tau$ (the greek letter tau).
