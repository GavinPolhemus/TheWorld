...
\startsolution
	This problem is just like the earlier example where Galileo lifted the pendulum (\in{ex.}[ex:GalileoPendulum1]), but now we must use the gravitational potential energy formula that works for large distances. The gravitational potential energy for the box and Earth is plotted in figure~\ref{fig:BoxEarthGravU}.
...
\startexample[ex:BlockonSpring] 
A block is suspended from a spring, as shown in \in{figure}[fig:BlockonSpring]. The spring's stiffness is $k = \unit{100.0 N/m}$. When the block is hanging motionless (no bouncing or swinging), the block's weight stretches the spring $\unit{2.94 cm}$, as shown. (Since upward is positive, the end of the stretched spring is at $y=\unit{-2.94cm}$.) Find the block's mass.
\startuseMPgraphic{graph::BlockonSpring}
SpringRadius := 5mm ;
SpringSlant := 0.4 ;
Coils := 10 ;
RelaxedLength := 1.6cm ;
...
pair Start, Stop ;
pickup pencircle scaled 0.8pt
draw (-1cm,0)--(2.5cm,0) withpen pencircle scaled 0.4pt withcolor 0.6 ;
draw (1cm,-Stretch)--(2.5cm,-Stretch) withpen pencircle scaled 0.4pt withcolor 0.6 ;
draw Spring((-1cm,RelaxedLength), (-1cm,0)) withpen pencircle scaled 1.6pt withcolor 0.4 ;
draw Spring((1cm,RelaxedLength), (1cm,-Stretch)) withpen pencircle scaled 1.6pt withcolor 0.4 ;
path Block ; Block := fullsquare scaled 2cm cornered .25cm shifted (1cm,-Stretch-1cm) ;
fill Block withcolor "gray" ;
draw Block ;
fill fullsquare xyscaled (5cm,.2cm) shifted (0.5cm,RelaxedLength+.1cm) withcolor "gray" ;
draw (-2cm,RelaxedLength) -- (3cm,RelaxedLength) ;
draw_marked((2.5cm,-Stretch-2.5cm)--(2.5cm,RelaxedLength), Stretch+2.5cm);
drawarrow (1cm,-Stretch)--(1cm,-Stretch+1.47cm) withpen pencircle scaled 1.6pt ;
drawarrow (1cm,-Stretch-1cm)--(1cm,-Stretch-2.47cm) withpen pencircle scaled 1.6pt ;
label.ulft("$F\sub{spring}$",(1cm,-Stretch));
label.llft("$F_g$",(1cm,-Stretch-1cm));
\stopuseMPgraphic

\startplacefigure[location=margin, reference=fig:BlockonSpring, title={A block hangs motionless from a spring. The net force on the block is zero.}]
\small
\reuseMPgraphic{graph::BlockonSpring}
\stopplacefigure

\startsolution
Since the block is motionless, its momentum remains zero. That is, $p\si = p\sf = 0$, even when we wait for some time $\Delta t > 0$. Use conservation of momentum.
\startformula\startmathalignment
\NC \cancel{p\si} + F\sn\,\Delta t \NC= \cancel{p\sf} \NR
\NC  F\sn \NC= 0 \NR
\stopmathalignment\stopformula
The forces on the block are gravity pulling down and the spring pulling up. (Since the spring is in the $y$ direction, Hooke's law becomes $F\sub{spring} = -ky$.)
\startformula\startmathalignment
\NC F_g + F\sub{spring} \NC= 0 \NR
\NC -mg - ky \NC= 0 \NR
\NC m \NC= \frac{-ky}{g} 
        = \frac{-(\unit{100.0N/m})(\unit{-0.0294m})}{\unit{9.8m/s^2}}
        = \unit{0.300 kg} \NR
\stopmathalignment\stopformula
The block's mass is $\unit{300g}$.
\stopsolution
\stopexample
As described in the last chapter, the potential energy function for real springs is not a perfect parabola (\at{p.}[fig:RealSpringGraphEnergy]). As a result, the potential energy's slope does not match Hooke's law for large displacements. Even so, Hooke's law is often an excellent approximation for limited displacements in many systems.
...
\startexample[ex:BlockonSpringMoon] 
The spring and block from \in{example}[ex:BlockonSpring] are taken to the Moon, and the experiment is repeated. When the block hangs motionless, what is the final position of the spring's bottom end?
The Moon's mass is $M\substar{\Moon} = \unit{7.34e22 kg}$. The Moon's radius is $r\substar{\Moon} = \unit{1.74e6 m}$.
\startsolution
As in \in{example}[ex:BlockonSpring], the net force on the block is zero.
The forces on the block are gravity pulling down and the spring pulling up, but we don't know the correct value for $g$ on the Moon, so we use Newton's law of universal gravitation to find the gravitational force on the $\unit{300g}$ block.
\startformula\startmathalignment
\NC F_g + F\sub{spring} \NC= 0 \NR
\NC -G\frac{mM\substar{\Moon}}{r^2} - ky \NC= 0 \NR
\NC y \NC= -G\frac{mM\substar{\Moon}}{kr^2} \NR
\stopmathalignment\stopformula
On the surface of the Moon, the distance from the Moon's center is simply the Moon's radius, $r = r\substar{\Moon} = \unit{1.74e6 m}$.
\startformula
  y = -G\frac{mM\substar{\Moon}}{kr\substar{\Moon}^2}
    = -\unit{6.67e-11 m^3/kg s^2}\frac{(\unit{0.300kg})(\unit{7.34e22 kg})}{(\unit{100n/m})(\unit{1.74e6 m})^2}
    = \unit{-0.0049m}
\stopformula
The spring stretches to $y = \unit{-0.49cm}$, as shown in \in{figure}[fig:BlockonSpringMoon].
\stopsolution
\startuseMPgraphic{graph::BlockonSpringMoon}
SpringRadius := 5mm ;
SpringSlant := 0.4 ;
Coils := 10 ;
RelaxedLength := 1.6cm ;
...
draw (-2cm,RelaxedLength) -- (3cm,RelaxedLength) ;
draw_marked((2.5cm,-Stretch-2.5cm)--(2.5cm,RelaxedLength), Stretch+2.5cm);
drawarrow (1cm,-Stretch)--(1cm,-Stretch+0.24cm) withpen pencircle scaled 1.6pt ;
drawarrow (1cm,-Stretch-1cm)--(1cm,-Stretch-1.24cm) withpen pencircle scaled 1.6pt ;
label.bot("$F\sub{spring}$",(1cm,-Stretch));
label.llft("$F_g$",(1cm,-Stretch-1cm));
\stopuseMPgraphic

\startplacefigure[location=margin, reference=fig:BlockonSpringMoon, title={The same block and spring from \in{figure}[fig:BlockonSpring], but now on the Moon. The net force is still zero, with both forces much smaller.}]
\small
\reuseMPgraphic{graph::BlockonSpringMoon}
\stopplacefigure

\stopexample
Calculations using Newton's universal law of gravitation can get a bit messy, as in the last example. We will find a cleaner approach later in this chapter.
...
\startexample[ex:gMoon] 
Find the gravitational field's strength $g$ on the Moon's surface.
The Moon's mass is $M\substar{\Moon} = \unit{7.34e22 kg}$. The Moon's radius is $r\substar{\Moon} = \unit{1.74e6 m}$.
\startsolution
On the surface of the Moon, the distance from the Moon's center is simply the Moon's radius, $r = r\substar{\Moon} = \unit{1.74e6 m}$.
\startformula
  g = \frac{GM\substar{\Moon}}{r^2}
    = \frac{(\unit{6.67e-11 kg m^3/s^2})(\unit{7.34e22 kg})}{(\unit{1.74e6 m})^2}
    = \unit{1.62 m/s^2}
\stopformula
The gravitational field on the Moon's surface is about one sixth as strong as here on Earth.
\stopsolution
\stopexample
...
\startsolution
As in \in{example}[ex:BlockonSpring], the net force on the block is zero.
Now, we do know the correct value for $g$ on the Moon (\in{example}[ex:gMoon]), so we can use the simple formula for gravitational force!
\startformula\startmathalignment
\NC F_g + F\sub{spring} \NC= 0 \NR
\NC -mg - ky \NC= 0 \NR
\NC y \NC= -\frac{mg}{k}
      = -\frac{(\unit{0.300kg})(\unit{1.62 m/s^2})}{\unit{100 N/m}} 
      = \unit{-0.0049m} \NR
\stopmathalignment\stopformula
The spring stretches to $y = \unit{-0.49cm}$, as in \in{example}[ex:BlockonSpringMoon].
\stopsolution
\stopexample
