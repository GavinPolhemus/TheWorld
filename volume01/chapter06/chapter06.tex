...
Newton's force can be found from the slope of the potential energy function on the energy graph. The force at a specific location is equal to \emph{minus} the slope of the potential energy at that location.
Lagrange preferred formulas to diagrams and graphs. Using the derivative, he expressed the relationship between potential energy and force in Lagrange's force formula.
\startformula
	F\sub{Newton} = -\frac{\partial}{\partial x} U
\stopformula

This derivative is calculated in exactly the same way as the derivatives in the last section. (After some examples, I will explain why each little $d$ has been replaced by a little $\delta$. This change does not affect the calculations, so ignore it for now.)
