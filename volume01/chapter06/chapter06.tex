...
Lagrange gave us a clue with his formula for converting Daniel Bernoulli's potential energy into Newtons force.
\startformula
...
Hamilton's general method brings together the big ideas of position, momentum, and energy in surprisingly simple way. Hamilton found that he did not need two separate formulas for kinetic energy and potential energy, one formula for the total energy is enough. (He chose the symbol $H$ for total energy, which we have adopted.) Hamilton also recognized momentum's importance, and used momentum in his formula for total energy. For example, he would write the total energy for a cart on a spring like this:
\startformula
  H = \frac{p^2}{2m} + \half kx^2
\stopformula
Euler and Lagrange used velocity rather than momentum, writing kinetic energy as $K=\onehalf mv^2$. Using velocity is reasonable, but it caused the update formulas to get tangled. Hamilton discovered that using momentum allowed him to separate the update formulas, making the equations of motion much easier understand and solve.
...
Hamilton followed Daniel Bernoulli's advice and described all interactions in terms of energy, not it terms of Newton's force. There is a bit of an obstacle to this approach. Lagrange's equation tells us how to find Newton's force from potential energy, but it does not tell us how to go the other way. In fact, some forces – like friction – cannot be described using potential energy and must be left outside the system. We will use $F$ only for these external forces. Any interaction that can be described using potential energy will be included in the energy formula.
...
\stopformula
The parts of this new formula should be reassuringly familiar. If our system has only external forces, no energy, this would be Newton's momentum update formula, $dp = F\,dt$. The new term on the right looks almost identical to Lagrange's equation, $F\sub{Newton} = -\partial U/\partial x$, but Hamilton replaces the potential energy $U$ with the total energy $H$. This replacement will be essential when using curved coordinates, like the polar coordinate used to describe orbits, but it has no effect in most of the systems we have studied.


When calculating $\partial H/\partial x$, we are looking at the virtual change $\partial H$ in the total energy due only to the virtual displacement 
...
\startformula
	dx = \frac{\partial H}{\partial p_x} dt
	\qquad
	dp = \left(-\frac{\partial H}{\partial x} + F \right) dt
\stopformula
