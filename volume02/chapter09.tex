...
\draw[ultra thick, fill=darkgray] (1.5,-0.26) rectangle node[above=2.5mm]{Anode} (2,0.26); % Anode
...
Batteries store electrical energy which can be used to run electrical devices. The energy is retrieved by allowing electrons to flow through a circuit from one end of the battery to the other. The voltage of the battery is a measure of how much energy is provided by each electron flowing through the circuit. A nine-volt battery, for example, provides nine electron volts ($9\units{eV}$) of energy for every electron that makes the journey. An electron volt is a very small amount of energy, about the amount of energy released in the reaction of a single molecule in the battery. Electron volts are related to our other unit of energy, the joule.
\startformula
	1\units{eV} = 1.60\sci{-19}\units{J}
\stopformula

A typical nine-volt battery has about $10^{22}$ electrons stored that can make the journey through the circuit, so the battery can provide about $9\sci{22}\units{eV}$ of energy, which is a significant amount of energy of energy.
\startformula
...
\startformula
	K = 5.0\sci{3}\units{\ucan{eV}}
		\left(\frac{1.60\sci{-19}\units{J}}{1\units{\ucan{eV}}}\right)
			= 8.0\sci{-16}\units{J}
\stopformula
Once the electrons pass the anode they no longer feel much force and will continue with \m{5.0\units{keV}} of kinetic energy until they run into something.
\stopsolution
\stopexample
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\startsolution
Five percent of the speed of light is \m{v = 1.50\sci{7}\units{m/s}}. This is very fast, but still slow enough that we can use the usual kinetic energy formula, \m{K=\onehalf\, mv^2}, where the mass of an electron is \m{m = 9.11\sci{-31}\units{kg}}. The electron's kinetic energy must be 
\startformula
...
While carefully connecting a \m{640\units{V}} power supply to your cathode ray tube, you wonder what the average force is on each electron as it is accelerated up to 10\% of the speed of light. The distance from the cathode to the anode is \m{3.0\units{cm}}. What is the average force?
	...
	\startaxis[
		...
		clip mode=individual,
	  	...
	  	\addplot[thick, samples=121, domain=0:10,]
		    {1.72-(0.2)*x}node[above right,pos=.7] {\m{U}}
		;
	  	\addplot[thick, samples=121, domain=0:10,]
		    {(0.2)*x}node[above left,pos=.3] {\m{K}}
		;
	  	\addplot[thick, samples=121, domain=0:10,]
		    {1.72}node[above,pos=.5] {\m{H=U+K}}
		;
	\stopaxis
\stoptikzpicture
\stopTEXpage
...
The \emph{electrical potential}, $V$, is a field that exists everywhere. At any location the electrical potential can be positive, negative, or zero. It is mostly zero, but electric charges alter the electrical potential. Positive charges produce a region of positive potential, while negative charges create a region of negative potential. Electrical potential obeys the superposition principle (just like waves), so that the potential at any location is the sum of the contributions coming from all of the charges in the area.
...
\section{Particles in Electric Fields}
Finding the electric force on a charge $q$ is easy with the \emph{electric field}, denoted by $\vec E$.
\startformula
	\vec F = q\vec E
\stopformula
...
The electric field is computed from the electric potential; it is not independent. The electric field vector points in the direction of decreasing electric potential. The electric field's magnitude is the magnitude of electric potential's slope in that direction. The vector's individual components are the potential's slope in the coordinate directions.
\startformula
...
\section{Magnetic field and magnetic force}
...
Mars, being somewhat smaller than Earth, cooled more quickly after the formation of the solar system, so it no longer has a molten core and therefore has lost its magnetic field, exposing it to the solar wind. The solar wind gradually blew Mars's atmosphere away, making it a rather inhospitable place. Most planets, as well as the Sun, have significant magnetic fields.

Like the electric field, the magnetic field exerts a force on charged particles, but only on changed particles that are moving. The direction of the magnetic force is \emph{perpendicular} to the direction of the field and also \emph{perpendicular} to the particle's direction of motion. This is described by a vector cross product.
\startformula
...
The direction of the magnetic field is determined using the right hand rule. This is a fully three dimensional exercise, so you will need to train with an experienced right-hand-rule practitioner to avoid sign errors and potential injury. The three vectors in the magnetic force formula are assigned to different fingers on your right hand. The vector $q\vec{v}$ is assigned to your index finger, as if you are firing the charge like an imaginary bullet out of your straight, extended finger. However, if $q$ is negative then you need to point your index finger in the direction \emph{opposite} the velocity vector $\vec{v}$, keeping it extended and straight while reversing your entire hand.
