...
\starttikzpicture% tikz code
	...
	\startaxis[
		...
		;
	\stopaxis
	\draw[ultra thick, darkgray, rounded corners, fill=gray] (potential plot.below south west) ++(-0.75,0) rectangle ++(7cm,-2cm); % Tube
	...
	\draw[thick,->] (potential plot.below south west) ++(1.5,-1) --node[below]{\m{F}} ++(1,0);
...
K\sf	 = (-1\units{e})(-2.5\units{kV} - 2.5\units{kV})
	...
	= 5.0\units{keV}
\stopformula
The electro passes the anode with kinetic energy \m{K=5.0\units{keV}}.
\stopsolution
\stopexample
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\stopformula
which agrees with the result in \in{example}[ex:CathodeForce].
